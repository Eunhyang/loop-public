...
async function migrateUsers() {
  ...
  for (const userDoc of usersSnapshot.docs) {
    ...
    // 올바른 만료일 계산
    const correctExpiryDate = calculateCorrectExpiryDate(activatedAt, durationMonths);
...
async function verifyExpiryDates() {
  ...
  for (const userDoc of usersSnapshot.docs) {
    ...
    // 예상 만료일 계산
    const expectedExpiryDate = calculateCorrectExpiryDate(activatedAt, durationMonths);
...
7. **Users with Missing Data**:
   - What happens: Some users might have `activatedAt` or `durationMonths` missing
   - How to handle: Skip these users with console warning, increment `skipCount`
   - Reason: Cannot recalculate without activation date and duration
