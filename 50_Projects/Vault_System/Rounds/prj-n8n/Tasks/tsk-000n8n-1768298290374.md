...
├── ac_batch_programs.yaml             # SSOT: Source configuration
```

**Implementation Details:**

**1. Source Configuration (YAML - SSOT):**
- Location: `public/50_Projects/Vault_System/Rounds/prj-n8n/Sources/ac_batch_programs.yaml`
- Structure:
  ```yaml
  sources:
    - name: "k-startup"
      ...
      parse:
        item_selector: "..."  # CSS selector for program items
        title_selector: "..."
        date_selector: "..."
        url_selector: "..."
    - name: "google"
  ...
  accelerators:
    ...
    - name: "Primer"
      search_enabled: true
    # ... (20 accelerators from plan)

  config:
    deadline_threshold_days: 14  # "Deadline Soon" window
    discord_webhook: "${DISCORD_WEBHOOK_URL}"
    schedule_cron: "0 15 * * *"  # 15:00 KST daily
  ```
...
2. **HTTP Request - Load YAML**: Fetch source config from vault file
3. **Code - Parse YAML**: Parse YAML into structured data
4. **HTTP Request - Fetch K-Startup**: Fetch K-Startup listing page
5. **Code - Parse K-Startup**: Extract program data using selectors from YAML
...
- Discord webhook URL (configured in YAML or n8n env var)
...
- [ ] Create YAML source configuration at `public/50_Projects/Vault_System/Rounds/prj-n8n/Sources/ac_batch_programs.yaml`
- [ ] Build n8n workflow JSON with all 19 nodes (+ error path) at `public/_build/n8n_workflows/ac_batch_daily_digest.json`
- [ ] Implement date extraction and normalization in "Parse K-Startup" node
- [ ] Implement Google search result parsing in "Parse Google Results" node
- [ ] Implement diff logic and classification in "Diff and Classify" node
- [ ] Implement redaction logic in "Redact Sensitive" node
- [ ] Implement Discord message formatting in "Format Discord Message" node
- [ ] Configure rate limiting for Google search (2s delay between requests)
...
- [ ] Update project.md to add this workflow to "현재 워크플로우" table

## 체크리스트

- [ ] YAML source config created
- [ ] n8n workflow JSON implemented
- [ ] Date parsing logic complete
- [ ] Diff and classification logic complete
