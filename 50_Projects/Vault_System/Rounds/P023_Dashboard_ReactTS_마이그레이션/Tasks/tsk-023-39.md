...
- tiptap
priority_flag: high
notes: "# Dashboard - 댓글 시스템 + @멘션 구현\n\n> Task ID: `tsk-023-39` | Project: `prj-023`\
  \ | Status: done\n\n## 목표\n\n**완료 조건**:\n\n1. OAuth 사용자 계정 생성 (김은향, 한명학) + Members\
  ...
  \ 기존 FastAPI, SQLAlchemy 사용 (추가 없음)\n\n### 프론트엔드\n\n- 이미 설치됨: `@tiptap/core`, `@tiptap/suggestion`,\
  ...
  \ 같은 entity_type/entity_id 내에서만 참조하도록 검증\n\n- [ ] 멘션된 엔티티 존재 여부 검증 (옵션)\n\n### API\
  ...
  \ Score (Realized Impact) 연동:\n\n- 댓글 데이터는 향후 Evidence 생성 시 참고 자료로 활용 가능\n- `mentions`\
  \ 필드로 어떤 엔티티가 논의되었는지 추적 가능\n\n---\n\n## 작업 로그\n\n### 2026-01-09\n\n**개요**: 댓글 시스템\
  \ + @멘션 기능 구현 완료\n\n**구현 내용**:\n\n**Backend (LOOP Vault API)**:\n\n- OAuth 계정 생성\
  \ 완료 (김은향, 한명학)\n- `members.yaml`에 email 필드 추가\n- `VaultCache.get_member_by_email()`\
  \ 메서드 추가\n- Comments API 구현:\n  - SQLite 테이블 생성 (comments 테이블)\n  - CRUD 엔드포인트:\
  \ GET/POST/PUT/DELETE `/api/comments`\n  - author 정보 자동 조회 (email → member)\n  -\
  \ mentions 파싱 및 저장\n- 파일: `api/routers/comments.py`, `api/models/comments.py`\n\n\
  **Frontend (Dashboard v2)**:\n\n- Tiptap MentionExtension 구현:\n  - `@` 트리거로 엔티티\
  \ 검색\n  - Task/Project/Member 자동완성 메뉴\n  - 키보드 네비게이션 (↑↓ Enter Esc)\n- CommentSection\
  \ 컴포넌트 구현:\n  - 댓글 목록 표시 (계층 구조 지원)\n  - 실시간 CRUD 연동\n  - React Query 캐싱\n- CommentEditor\
  ...
  **Infrastructure**:\n\n- NAS 서버 rebuild 완료 (`/mcp-server rebuild`)\n- API 캐시 갱신\
  \ 완료\n- Frontend build & deploy 완료\n\n**검증 완료**:\n\n- OAuth 계정 로그인 테스트\n- 댓글 CRUD\
  \ 동작 확인\n- @멘션 팝업 및 자동완성 동작 확인\n- 멘션 링크 클릭 시 네비게이션 확인\n- Task Drawer에서 댓글 섹션 정상\
  \ 표시\n\n**최종 상태**: done\n\n**남은 작업 (후속 개선)**:\n\n- Codex Review Suggestions 항목들\
  \ (보안, pagination, UI/UX 개선)\n- 댓글 알림 기능 (향후 Task로 분리)"
---
...
### 2026-01-09
**개요**: 댓글 시스템 + @멘션 기능 구현 완료

**구현 내용**:

**Backend (LOOP Vault API)**:
- OAuth 계정 생성 완료 (김은향, 한명학)
- `members.yaml`에 email 필드 추가
- `VaultCache.get_member_by_email()` 메서드 추가
- Comments API 구현:
  - SQLite 테이블 생성 (comments 테이블)
  - CRUD 엔드포인트: GET/POST/PUT/DELETE `/api/comments`
  - author 정보 자동 조회 (email → member)
  - mentions 파싱 및 저장
- 파일: `api/routers/comments.py`, `api/models/comments.py`
...
- @멘션 팝업 및 자동완성 동작 확인
- 멘션 링크 클릭 시 네비게이션 확인
- Task Drawer에서 댓글 섹션 정상 표시

**최종 상태**: done

**남은 작업 (후속 개선)**:
- Codex Review Suggestions 항목들 (보안, pagination, UI/UX 개선)
- 댓글 알림 기능 (향후 Task로 분리)
...
**문제 발견**:
- 댓글 수정 버튼 클릭 시 빈 화면이 표시됨
- CommentEditor에 initialContent prop이 전달되지 않음

**구현 내용**:
- CommentEditor에 `initialContent` prop 추가
- CommentItem에서 수정 시 `comment.content` 전달
- 파일: `src/features/comments/CommentEditor.tsx`, `src/features/comments/CommentItem.tsx`

**검증 완료**:
- 댓글 수정 버튼 클릭 시 기존 내용 표시 확인
- 수정 저장 후 정상 업데이트 확인
- Dashboard 로컬 빌드 성공

**배포 완료**:
- 코드 커밋 및 GitHub 푸시 완료 (commit: 9457dca)
- Dashboard 빌드 완료 (dist/)
- NAS 배포 완료 (docker cp → loop-api container)
- 프로덕션 확인 완료 (https://mcp.sosilab.synology.me/v2/)
