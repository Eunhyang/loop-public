...
> Task ID: `tsk-023-12` | Project: `prj-023` | Status: done
...
#### 2026-01-08
**개요**: Task 완료 - Kanban Visibility 필터 4종 모두 정상 작동

**구현 내용**:
- Member 타입에 `active?: boolean` 필드 추가
- `filterTasksByMembers()` 함수 구현 (active + role 기반 필터링)
- Kanban 페이지 하드코딩 제거, useCombinedFilters 훅 사용
- buildKanbanColumns 시그니처에 members 파라미터 추가

**검증 결과**:
- Codex 최종 리뷰 통과 (타입 안전성, 로직 정확성 확인)
- 4개 Visibility 필터 모두 작동 (Inactive Members, Non-Core Members, Inactive Projects, Inactive Tasks)
