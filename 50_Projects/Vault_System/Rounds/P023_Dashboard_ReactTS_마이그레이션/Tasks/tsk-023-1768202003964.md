...
### 2026-01-12 - Hotfix: UI 하드코딩 누락 수정

**문제 발견**:
- prj-007, prj-015 (status: done) 프로젝트의 태스크가 Kanban에서 안 보임
- 원인 분석:
  1. `ProjectFilters.tsx` line 4에 하드코딩 남아있음: `['planning', 'active', 'paused', 'completed', 'cancelled']`
  2. SSOT (schema_constants.yaml): `['planning', 'doing', 'done', 'hold', 'cancelled']`
  3. UI 버튼과 실제 데이터 불일치로 필터링 실패

**근본 원인**:
- 이전 세션에서 `filterDefaults.ts`와 selectors만 수정
- **`ProjectFilters.tsx` UI 컴포넌트 수정 누락**
- UI에서 `done` 버튼이 안 보여서 사용자가 켤 수 없었음

**수정 내용**:

1. **`src/features/filters/components/FilterPanel/ProjectFilters.tsx`**:
   - 하드코딩된 `PROJECT_STATUSES`, `PRIORITIES` 배열 제거
   - `useConstants()` hook 추가
   - `constants.project.status`, `constants.priority.values` 사용
   - `status_labels`, `priority.labels`로 버튼 텍스트 표시

2. **`src/constants/filterDefaults.ts`**:
   - line 52의 `done` 제외 로직 제거
   - Before: `constants.project.status.filter(s => s !== 'completed' && s !== 'done')`
   - After: `constants.project.status` (전체 사용)

**결과**:
- ✅ 빌드 성공 (`npm run build`)
- ✅ NAS Docker 컨테이너에 배포 완료
- ✅ Project Status 필터에 SSOT 값 표시: `planning, doing, done, hold, cancelled`
- ✅ prj-007, prj-015 태스크 표시 가능 (done 필터 활성화 시)

**교훈**:
- Constants SSOT 연동 시 **모든 UI 컴포넌트**까지 확인 필요
- selectors/hooks만 수정하고 UI 버튼 하드코딩을 놓치면 실제 동작 안 함
