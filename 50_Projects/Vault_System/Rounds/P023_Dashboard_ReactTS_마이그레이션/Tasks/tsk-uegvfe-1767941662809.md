...
type: dev
target_project: loop
notes: '# Dashboard - Task Drawer PR/Commit 필드 추가


  ## 설명


  dev 타입 Task의 PR/commit 연동 정보를 Dashboard Task Drawer에서 볼 수 있도록 `pr_url`, `merged_commit`
  필드를 추가한다.
  ...
  | `api/models/entities.py` | API 데이터 모델 | TaskUpdate, TaskCreate에 pr_url, merged_commit
  추가 |
  ...
  **Edit 모드:**
  ...
  - [x] API 모델 수정 - TaskCreate, TaskUpdate에 pr_url, merged_commit 추가


  - [x] API routers/tasks.py - create_task, update_task에 필드 persist 추가


  - [x] task-panel.js View 모드 - renderReadOnlyHTML()에 GitHub 섹션 추가


  - [x] task-panel.js Edit 모드 - open(), openNew(), save()에 필드 추가


  - [x] task-panel.js XSS 보안 - inline onclick 제거, event listener 사용


  - [x] Codex code review 통과 - 모든 critical issues 수정 완료
  ...
  **개요**: Task 완료 - Dashboard Task Drawer에 PR/Commit 필드 추가 완료


  **결과**:

  - API 모델 (`api/models/entities.py`) - TaskCreate, TaskUpdate에 `pr_url`, `merged_commit`
  필드 추가

  - API 라우터 (`api/routers/tasks.py`) - create_task, update_task에 필드 persist 로직 추가

  - Dashboard UI (`_dashboard/js/components/task-panel.js`) - View 모드에 GitHub Integration
  섹션 추가

  - Dashboard UI - Edit 모드에 PR URL, Commit SHA 입력 필드 추가

  - XSS 보안 강화 - inline onclick 제거, event listener 사용

  - Codex code review 통과 - 모든 critical issues 수정 완료
...
**결과**:
- API 모델 (`api/models/entities.py`) - TaskCreate, TaskUpdate에 `pr_url`, `merged_commit` 필드 추가
- API 라우터 (`api/routers/tasks.py`) - create_task, update_task에 필드 persist 로직 추가
- Dashboard UI (`_dashboard/js/components/task-panel.js`) - View 모드에 GitHub Integration 섹션 추가
- Dashboard UI - Edit 모드에 PR URL, Commit SHA 입력 필드 추가
- XSS 보안 강화 - inline onclick 제거, event listener 사용
- Codex code review 통과 - 모든 critical issues 수정 완료
