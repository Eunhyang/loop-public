...
#### 2026-01-16 03:30
**개요**: Hypothesis Candidates API 구현 완료

**변경사항**:
- 개발: `api/models/hypothesis_candidates.py` - Pydantic 응답 모델 (CandidateItem, CandidatesGroup, Recommendation, HypothesisCandidatesResponse)
- 개발: `api/utils/text_similarity.py` - TF-IDF 텍스트 유사도 엔진 (싱글톤 패턴, 코퍼스 캐싱)
- 개발: `api/services/hypothesis_candidates_service.py` - 후보 생성 로직 (3가지 소싱 방법: track_based, condition_based, keyword_based)
- 수정: `api/routers/mcp_composite.py` - 새 엔드포인트 추가 `GET /api/mcp/project/{id}/hypothesis-candidates`
- 수정: `pyproject.toml` - scikit-learn>=1.3.0,<2.0 의존성 추가

**파일 변경**:
- `api/models/hypothesis_candidates.py` (신규 78줄) - Pydantic 모델, max_items로 리스트 길이 검증
- `api/utils/text_similarity.py` (신규 182줄) - TF-IDF 엔진, 빈 ID 필터링, 코퍼스 캐싱
- `api/services/hypothesis_candidates_service.py` (신규 518줄) - 후보 생성 서비스, 중복 제거, LLM 리랭크
- `api/routers/mcp_composite.py` (+45줄) - 엔드포인트, 에러 핸들링
- `pyproject.toml` (+1줄) - scikit-learn 의존성

**Codex 리뷰**: ✅ 통과 (이슈 6개 수정)
- 코퍼스 refresh 로직 수정 (time.time() → hypothesis count 기반)
- max_length → max_items 변경 (Pydantic list 검증)
- 빈 hypothesis_id 필터링 추가
- evidence_status 타입 가드 추가
- response_model 추가 (FastAPI 계약 안전성)
- LLM fallback 로직 개선 제안 (향후 개선 가능)

**결과**: ✅ 빌드 성공, 코드 push 완료 (배포는 NAS 인증 이슈로 수동 필요)

**커밋**: `274d61a100b3fe3a2e67eee8130e390318b3fcb6` on branch `task/tsk-023-1768500289204`

**엔드포인트 사용법**:
```bash
# 기본 호출 (규칙 기반)
GET /api/mcp/project/prj-023/hypothesis-candidates

# LLM 리랭크 사용
GET /api/mcp/project/prj-023/hypothesis-candidates?include_llm_rerank=true&provider=openai&max_candidates=10
```
...
```json
{
  ...
  "conditions_3y": ["cond-e"],
  "candidates": {
    "track_based": [{"hypothesis_id": "hyp-2-01", "score": 0.85, ...}],
    "condition_based": [...],
    "keyword_based": [...]
  },
  "recommendation": {
