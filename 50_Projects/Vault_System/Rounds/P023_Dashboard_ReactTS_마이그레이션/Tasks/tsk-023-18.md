...
> Task ID: `tsk-023-18` | Project: `prj-023` | Status: done
...
#### 2026-01-08
**개요**: Task 완료 - Child Project 필터 버그 수정

**수정 내용**:
- `useUrlFilters.ts` Line 88 삭제: `setProjectIds`에서 `newParams.delete('program')` 제거
- `useUrlFilters.ts` Line 101 삭제: `toggleProjectId`에서 `newParams.delete('program')` 제거

**결과**:
- Program 버튼 클릭 후 하위 Project 버튼 클릭 시 버튼들이 정상적으로 유지됨
- Child Project 필터링 정상 작동
- 버튼 토글(선택 해제) 기능 정상 작동
