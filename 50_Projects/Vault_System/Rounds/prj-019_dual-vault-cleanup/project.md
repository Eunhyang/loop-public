...
### 성공 기준
1. [x] exec vault의 `50_Projects_Exec/` 구조를 `50_Projects/`로 마이그레이션 완료
2. [x] exec CLAUDE.md 문서 업데이트 완료
3. [x] public과 동일한 API 경로 패턴 사용 가능 (예: `/api/projects`, `/api/tasks`)
4. [ ] 기존 기능 정상 동작 확인 (Validation 필요)
...
exec vault가 `50_Projects_Exec/P015/` 구조를 사용하고 있어 public vault의 `50_Projects/` 구조와 불일치했음.
이로 인해:
- API 라우팅 로직 중복
- 스크립트 재사용 어려움
- CLAUDE.md 문서 복잡화
...
**핵심 요구사항** (모두 완료):
1. [x] 디렉토리 이름 `50_Projects_Exec/` → `50_Projects/` 변경
2. [x] 내부 파일 링크 업데이트
3. [x] exec CLAUDE.md 문서 업데이트
4. [x] `40_People/Hiring_Rounds/` 예시 정리
