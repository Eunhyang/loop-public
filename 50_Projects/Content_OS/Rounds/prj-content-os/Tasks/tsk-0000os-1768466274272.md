...
#### Phase 1: Setup & Types
- [ ] Create feature folder structure under `src/features/content-os/pipeline/`
- [ ] Add `PipelineTask` types to `src/types/content-os.ts`
- [ ] Add query key `contentOsTasks(filters)` to `src/queries/keys.ts`
- [ ] Create `api.ts` with `fetchTasks` and `updateTaskStatus` functions

#### Phase 2: API Integration
- [ ] Implement `fetchTasks()` using `src/services/http.ts`
- [ ] Implement `updateTaskStatus(taskId, status)` mutation
- [ ] Create React Query hooks in `queries.ts`:
  - [ ] `useTasksQuery(filters?)`
  - [ ] `useUpdateTaskStatusMutation()`
- [ ] Test API integration with curl/Postman

#### Phase 3: Component Migration
- [ ] Port `PipelineBoard.tsx` - replace `@dnd-kit` with `@hello-pangea/dnd`
- [ ] Port `PipelineColumn.tsx` - update for new DnD library
- [ ] Port `PipelineCard.tsx` - remove Next.js imports
- [ ] Port `TaskDetailModal.tsx` - use react-router navigation
- [ ] Update all component imports to use Dashboard V2 paths

#### Phase 4: Page Integration
- [ ] Create `src/pages/ContentOS/Pipeline/index.tsx`
- [ ] Add route to React Router config: `/content-os/pipeline`
...
- [ ] Set up `DragDropContext` in PipelineBoard
- [ ] Implement `onDragEnd` handler with optimistic update
...
- [ ] Verify no Next.js imports remain
