...
#### 2026-01-15 19:05
**개요**: Content OS 데이터 API를 FastAPI로 이관하고 프론트 훅을 API 기반으로 전환

**변경사항**
- 개발: `api/routers/content_os/data.py`에 opportunities/contents/tasks 조회 API 추가 (필터/정렬/limit/cursor, Firestore Admin 사용)
- 개발: `apps/content-os/lib/api/http.ts` API fetch 헬퍼 추가 (API 토큰/쿠키 포함, 노캐시)
- 수정: `api/main.py`에 Content OS 데이터 라우터 등록
- 수정: `useOpportunities/useContents/useTasks` 훅을 Firebase 구독 → API 폴링 기반으로 전환 (`fetchApi`, 60s refetch)

**테스트**
- `python -m py_compile api/routers/content_os/data.py`

**커밋**: `9cf29708c` on branch `task/tsk-0000os-1768466086422`
