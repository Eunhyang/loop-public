...
Write Targets를 `schema_constants.yaml`에 SSOT로 편입하고, 단일 로더(ssot_loader.py)로 YAML 접근을 통일하여 드리프트를 원천 차단.

## 체크리스트

- [x] Phase 0: schema_constants.yaml에 write_targets 섹션 추가
- [x] Phase 1: shared/ssot_loader.py 신규 생성
- [x] Phase 2: constants.py, validate_schema.py, build_graph_index.py 마이그레이션
- [x] Phase 3: API 라우터에 검증 로직 추가
- [x] Phase 4: SSOT_CONTRACT.md 문서 역할 분리
- [x] Phase 5: build_ssot_docs.py 생성 및 Derived 문서 생성
- [x] 검증 및 테스트
...
### Phase 0: YAML 준비
`schema_constants.yaml`에 `write_targets.writable_fields` 섹션 추가 (v5.11):
...
### Phase 1: SSOT Loader 통일
`code/shared/ssot_loader.py` 신규 생성:
- `load_schema_constants()`: 캐싱된 YAML 로드 (lru_cache)
- `get_writable_fields(entity_type)`: 엔티티별 수정 가능 필드 반환
- `get_known_fields(entity_type)`: 엔티티별 알려진 필드 반환
- `validate_writable_subset()`: writable ⊆ known 일관성 검증
- `clear_cache()`: 테스트/개발용 캐시 초기화

### Phase 2: 기존 코드 마이그레이션
직접 YAML 로드 → ssot_loader 사용으로 변경:
- `api/constants.py`: `_load_constants()` 제거, ssot_loader import
- `scripts/validate_schema.py`: `_load_schema_constants()` → ssot_loader 호출
- `scripts/build_graph_index.py`: `_load_schema_constants()` → ssot_loader 호출

### Phase 3: API 라우터 검증 강제
`shared/validators.py` 신규 생성:
- `validate_update_payload(entity_type, payload)`: 2단 검증
  - 1단: known_fields 체크 → UNKNOWN_FIELD (400)
  - 2단: writable_fields 체크 → WRITE_TARGET_VIOLATION (400)

적용 라우터:
- `api/routers/projects.py`: update_project에 검증 추가
- `api/routers/tasks.py`: update_task에 검증 추가
- `api/routers/hypotheses.py`: update_hypothesis에 검증 추가, PATCH 엔드포인트 추가

### Phase 4: 문서 역할 분리
`SSOT_CONTRACT.md` Section 6.3 수정:
- 값 테이블 제거
- YAML 참조 안내로 전환
- Derived 문서 링크 추가

### Phase 5: Derived 문서 자동 생성
`scripts/build_ssot_docs.py` 신규:
- YAML에서 writable_fields 읽어 `00_Meta/SSOT_WRITE_TARGETS.md` 자동 생성
- 엔티티별 Writable/Read-Only 필드 목록 포함

### 생성된 파일
```
code/shared/ssot_loader.py       # YAML 단일 로더 (291줄)
code/shared/validators.py        # API 검증 유틸 (101줄)
code/scripts/build_ssot_docs.py  # Derived 문서 생성 (160줄)
public/00_Meta/SSOT_WRITE_TARGETS.md  # 자동 생성 문서
```

### 검증 결과
```
1. YAML Load Test: PASS
2. Task writable fields: ['status', 'assignee', 'due', ...]
3. Project writable fields: ['status', 'owner', 'start_date', ...]
4. Writable ⊆ Known validation: PASS
5. Validators test: PASS (valid/unknown/readonly 모두 정상 처리)
6. pre-commit hooks: PASS
```

### 커밋 정보
- Branch: `feature/ssot-write-targets` → main 머지 완료
- Commit: `336fb82a0`
- 변경: 9 files, +630 insertions, -33 deletions

## 짧은 회고

PRD에서 정의한 대로 SSOT 중앙화 완료. 이제 새 필드 추가 시 YAML 1곳만 수정하면 API가 자동으로 허용하고, 문서도 자동 생성됨. 드리프트 원천 차단 달성.
