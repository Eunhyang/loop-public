...
#### 2026-01-08 23:45
**개요**: Dashboard Create Buttons 버그 수정 완료 - TaskForm create 모드 구현, ProjectForm/ProgramForm footer 하단 고정, API 에러 처리 개선

**변경사항**:
- 개발: TaskForm create 모드 전체 구현 (placeholder 제거)
- 수정: ProjectForm/ProgramForm CSS 구조 변경 (footer 하단 고정)
- 수정: API 에러 처리 개선 (error.detail 지원 추가)

**파일 변경**:
- `src/features/tasks/components/TaskForm.tsx`
  - create 모드 placeholder 제거
  - 실제 폼 UI 구현 (entity_name, project_id, assignee, priority, status, type)
  - useCreateTask hook 연동
  - 에러 처리 및 로딩 상태 관리
  - footer 하단 고정 CSS 적용

- `src/features/projects/components/ProjectForm.tsx`
  - form 구조 변경: `flex flex-col h-full`
  - fields 영역: `flex-1 overflow-y-auto`
  - footer 영역: `flex-shrink-0` (하단 고정)
...
- `src/features/tasks/queries/useTasks.ts`
  - useCreateTask 에러 처리 개선
  - error.response?.data?.detail 지원 추가

- `src/features/projects/queries/useProjects.ts`
  - useCreateProject 에러 처리 개선
  - error.response?.data?.detail 지원 추가

- `src/features/programs/queries/usePrograms.ts`
  - useCreateProgram 에러 처리 개선
  - error.response?.data?.detail 지원 추가

**검증 결과**:
- ✅ TaskForm: create 모드에서 placeholder 대신 실제 폼 표시
- ✅ ProjectForm: footer 하단 고정 확인
- ✅ ProgramForm: footer 하단 고정 확인
- ✅ 빌드 성공: `npm run build`
- ✅ API 에러 메시지 정상 표시

**알려진 이슈**:
- Program 생성 후 필터에 즉시 반영 안 됨 (별도 Task로 분리 예정)
