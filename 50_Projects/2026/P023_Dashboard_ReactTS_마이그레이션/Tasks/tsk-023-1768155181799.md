...
> Task ID: `tsk-023-1768155181799` | Project: `prj-023` | Status: done
...
### 검증 체크리스트
- [x] Tier/Magnitude/Confidence 수정 가능
- [x] 실시간 점수 업데이트 (0.32/3.0 형식)
- [x] "?" 클릭 시 설명 모달
- [x] reasoning 표시
- [x] 모든 Track 버튼으로 표시
- [x] Track 선택/해제 + weight 입력
- [x] Track 클릭 → 오른쪽 패널
- [x] weight 합계 표시 (≤100% 검증)
- [x] Validates 드롭다운 multi-select
- [x] Primary Hypothesis 드롭다운 single-select
- [x] Hypothesis 클릭 → 오른쪽 패널
...
- `features/impact/` 모듈 생성 (7개 컴포넌트 + 타입 + 유틸)
- `ProjectForm.tsx` 통합
- 빌드 통과

### 2026-01-12 (Bug Fixes - Session 2)

#### 1. Hypothesis 클릭 시 오른쪽 패널 안 열림
- **문제**: 편집 모드에서 hypothesis 칩 클릭 시 선택만 토글되고 `onHypothesisClick` 미호출
- **수정**: `HypothesisSelector.tsx`에 `→` 버튼 추가하여 상세 보기 분리
  - 칩 클릭: 선택/해제 토글
  - `→` 버튼 클릭: 오른쪽 패널 열기

#### 2. Reasoning 표시 안됨
- **문제**: API가 `reasoning['tier']`, `reasoning['summary']` 등 개별 필드로 반환하는데, 프론트엔드가 `reasoning['expected_impact']` 찾음
- **수정**: `ProjectForm.tsx`에 reasoning 조합 헬퍼 함수 추가
  - `getExpectedImpactReasoning()`: summary 우선, 없으면 개별 필드 조합
  - `getTrackContributesReasoning()`: contributes/track_contributes
  - `getValidatesReasoning()`: validates/primary_hypothesis_id

#### 3. Tier/Magnitude 선택값 표시 안됨
- **문제**: API(SSOT: `impact_model_config.yml`)와 프론트엔드 값 불일치
  - Tier: API `enabling` vs Frontend `tactical`
  - Magnitude: API `mid` vs Frontend `medium`
- **수정**: 프론트엔드를 SSOT와 일치시킴
  - `types/project.ts`: `tactical` → `enabling`, `medium` → `mid`
  - `features/impact/types.ts`: `ImpactTier`, `ImpactMagnitude` 수정
  - `features/impact/utils/calculator.ts`: `MAGNITUDE_POINTS`, OPTIONS 수정
  - `ExpectedImpactEditor.tsx`: colors, defaults 수정
  - `ImpactExplainerModal.tsx`: 테이블, 설명 수정

### 수정된 파일 목록
```
dashboard-v2/src/
├── types/project.ts                                    # ExpectedImpact 타입
├── features/impact/
│   ├── types.ts                                        # ImpactTier, ImpactMagnitude
│   ├── utils/calculator.ts                             # MAGNITUDE_POINTS, OPTIONS
│   └── components/
│       ├── ExpectedImpactEditor.tsx                   # colors, defaults
│       ├── HypothesisSelector.tsx                     # → 버튼 추가
│       └── ImpactExplainerModal.tsx                   # 테이블, 설명
└── features/projects/components/ProjectForm.tsx       # reasoning 헬퍼
```
