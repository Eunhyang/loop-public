...
#### 2026-01-16 15:58
**개요**: n8n → /suggest-batch 직접 호출 구조로 변경, pending review 생성 기능 통합

**변경사항**:
- 개발: `SuggestBatchRequest`에 `actor`, `source_workflow` 필드 추가
- 개발: `SuggestBatchSuggestion`에 `entity_name` 필드 추가
- 개발: `suggest_batch` 함수에 mode="pending" 시 pending review 생성 로직 추가
- 수정: Codex 리뷰 반영 - `apply_patch.fields` → `model_dump()` 사용
- 수정: Codex 리뷰 반영 - 스키마 정렬 (`pending_reviews` → `reviews`, `review_id` → `id`, `workflow_id` → `source_workflow`)

**파일 변경**:
- `api/models/impact_batch.py`: SuggestBatchRequest, SuggestBatchSuggestion 모델 확장 (12줄 추가)
- `api/routers/impact_batch.py`: suggest_batch 함수에 pending review 생성 로직, entity_name 설정 (50줄 추가)

**Codex 리뷰**: ✅ 통과 (2회 리뷰, 속성 접근 오류 + 스키마 불일치 수정)

**결과**: ✅ 빌드 성공, 배포 완료, API health check 정상

**커밋**: `401d53633` on branch `task/tsk-023-1768545831670`
