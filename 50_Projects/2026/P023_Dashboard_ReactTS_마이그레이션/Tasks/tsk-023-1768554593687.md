...
- [x] T1: forms 모듈 생성 (types, hook, utils, components)
- [x] T2: ProjectFormLegacy 리네임
- [x] T3: 새 ProjectForm 구현 (모드별 처리, review 호환)
- [x] T4: EntityDrawer import 수정
- [x] T5: 빌드 검증
- [ ] T6: 수동 테스트
...
#### 2026-01-16 19:11
**개요**: Entity Form 패턴 통일 - forms 모듈 생성 및 ProjectForm 마이그레이션 완료

**변경사항**:
- 개발: `features/forms/` 모듈 신규 생성
  - `useEntityFormState` 훅 (로컬 상태 + dirty 추적 + 저장)
  - `FormActions` 컴포넌트 (Save/Cancel/Delete 버튼 바)
  - `diffObject`, `shallowEqual` 유틸 (변경 감지용)
- 수정: `ProjectForm.tsx` 전면 리팩토링 (useEntityFormState 적용)
- 보존: `ProjectFormLegacy.tsx` (롤백용)

**파일 변경**:
- `dashboard-v2/src/features/forms/types.ts` - 타입 정의
- `dashboard-v2/src/features/forms/hooks/useEntityFormState.ts` - 핵심 훅
- `dashboard-v2/src/features/forms/utils/serialize.ts` - diff 유틸
- `dashboard-v2/src/features/forms/components/FormActions.tsx` - 버튼 바
- `dashboard-v2/src/features/forms/index.ts` - barrel export
- `dashboard-v2/src/features/projects/components/ProjectForm.tsx` - 새 구현
- `dashboard-v2/src/features/projects/components/ProjectFormLegacy.tsx` - 기존 보존

**빌드**: ✅ `npm run build` 통과

**결과**: ✅ main 머지 완료, GitHub push 완료

**커밋**: `0baa9f7de` on branch `task/tsk-023-1768554593687`
