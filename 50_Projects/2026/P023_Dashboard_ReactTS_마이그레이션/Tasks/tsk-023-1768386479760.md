...
- Frontend – Notion 스타일 AI 카드
  ...
  - Expected Impact 카드(create/edit/viewable):
    ...
    - **CRITICAL - Finding 1 Fix**: `즉시 적용` must call `updateProject({ expected_impact: overrideOutput })` mutation to persist the applied state. Do NOT rely on cache invalidation alone - the server must receive the update via the API endpoint (e.g., `PATCH /api/projects/{id}`). After successful mutation, invalidate relevant queries.
    - **Finding 2 Fix**: For 'reason' kind messages in the chat flow, use `diff_summary` field from `ExpectedInferResponse` as the reasoning text. If reasoning display is required beyond diff_summary, coordinate with backend to add a dedicated `reasoning` field to the response schema.
...
- **Finding 3 Fix**: `impactChatMessages` conversation state is preserved within the drawer session only (panel show/hide within the same drawer mount). When the drawer unmounts (user navigates away or closes drawer), the conversation history is lost. This is intentional for the current implementation - no cross-session persistence is required. If future requirements demand persistent conversation history across drawer reopens, specify storage mechanism (React Query cache, localStorage, or backend storage).
...
- [ ] Backend: Ensure `ExpectedInferResponse` includes `diff_summary` field for reasoning display (Finding 2)
...
- [ ] Frontend: **CRITICAL** - Implement `updateProject` mutation call in `즉시 적용` handler (Finding 1) - must persist to server via PATCH endpoint, not just invalidate cache
- [ ] Frontend: Use `diff_summary` from API response for 'reason' kind messages in chat (Finding 2)
- [ ] Frontend: Document that `impactChatMessages` is session-only (drawer mount lifecycle) - no persistence across unmount (Finding 3)
...
- [ ] Tests: Verify `즉시 적용` persists data via API call, not cache mutation alone
