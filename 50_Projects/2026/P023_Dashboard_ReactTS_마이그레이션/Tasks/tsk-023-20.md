...
> Task ID: `tsk-023-20` | Project: `prj-023` | Status: done
...
#### Task Completion (2026-01-08)

**개요**: Task 완료 - FilterPanel 기본 필터 활성화 상태 개선 완료

**결과**:
- 모든 구현 완료 (filterDefaults, selectors, useCalendarEvents, localFilterStore, useFilterIndicator, projects/selectors)
- Codex 최종 리뷰 통과 (Phase 6)
- 빈 배열 → 전체 선택 시맨틱 구현
- localStorage 마이그레이션 로직 추가 (버전 플래그 포함)
- Filter indicator 버그 수정 (배열 mutation 방지)
- Project priority 필터 버그 수정 (null priority 처리)

**최종 상태**: done

**Manual Testing Required**:
- 초기 로드 시 모든 버튼 활성화 확인
- 버튼 클릭 시 비활성화 동작 확인
- Reset All 동작 확인
- Calendar 페이지 필터링 동작 확인
- 기존 사용자 마이그레이션 동작 확인
