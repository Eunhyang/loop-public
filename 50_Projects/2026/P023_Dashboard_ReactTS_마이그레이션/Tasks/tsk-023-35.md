...
> Task ID: `tsk-023-35` | Project: `prj-023` | Status: done
...
**개요**: Task 완료 - ChipSelectExpand 구현 완료

**구현 내용**:
- ChipSelectExpand 컴포넌트 생성 (Core 멤버 + 더보기 패턴)
- chipColors.ts에 memberColor, projectColor, unassignedColor 추가
- TaskForm에 assignee, project 필드 적용
- ProjectForm에 owner 필드 적용
- 빌드 성공 확인

**검증**:
- npm run build 성공 (warnings는 chunk size 관련으로 기능에 영향 없음)
- ChipSelectExpand.tsx 생성 확인
- chipColors.ts 업데이트 확인
