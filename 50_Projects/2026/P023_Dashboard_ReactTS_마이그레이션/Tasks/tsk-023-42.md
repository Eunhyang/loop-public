...
> Task ID: `tsk-023-42` | Project: [[prj-023]] | Status: done
...
### 2026-01-09
**개요**: Task 완료 - ProjectForm view 모드 리팩토링 완료

**작업 내용**:
- isReadOnly 플래그 추가 (line 19: `const isReadOnly = mode === 'view'`)
- View 모드 early return 제거 (통합 UI 구조로 변경)
- Edit/View 모드 통합 (line 154: `if (mode !== 'create')`)
- Tasks 섹션 항상 표시 (lines 320-397)
- MarkdownEditor readOnly prop 적용 (line 429)
- Hypothesis 섹션 isReadOnly 조건부 렌더링 (line 404)

**결과**:
- View 모드에서 Tasks, Description, Hypothesis 모두 표시됨
- 모든 필드가 isReadOnly 플래그에 따라 read-only 또는 편집 가능하게 동작
- 코드 중복 제거 (view/edit 통합으로 약 60 lines 감소)
- TaskForm 패턴과 일관성 유지
