...
> Task ID: `tsk-023-31` | Project: `prj-023` | Status: done
...
#### 2026-01-09
**개요**: EntityDrawer 네비게이션 스택 구현 완료

**변경사항**:
- 개발:
  - UiContext에 drawerStack 상태 추가 (배열 기반 스택)
  - pushDrawer, popDrawer, canGoBack 함수 구현
  - DrawerShell에 뒤로가기 버튼 UI 추가
  - TaskForm 등에서 entity 클릭 시 pushDrawer 사용
- Codex 리뷰:
  - 6개 제안사항 검토 완료 (모두 고려사항, Critical 이슈 없음)
  - Edge case 처리 확인 (빈 스택, unsaved state, 메모리 등)

**핵심 구현**:
- UiContext: `drawerStack: ActiveEntityDrawer[]` 상태로 네비게이션 스택 관리
- DrawerShell: `showBackButton`, `onBack` props로 뒤로가기 UI 제공
- 사용자 플로우: Task → Project → Track → 뒤로가기 × 2 → Task

**결과**: ✅ 빌드 성공 (dashboard-v2)
