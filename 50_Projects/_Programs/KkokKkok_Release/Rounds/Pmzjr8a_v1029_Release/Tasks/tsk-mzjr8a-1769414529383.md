---
entity_type: Task
entity_id: tsk-mzjr8a-1769414529383
entity_name: Recovery - ILOS 데이터 레이어 구현
created: '2026-01-26'
updated: '2026-01-28'
status: doing
project_id: prj-mzjr8a
parent_id: prj-mzjr8a
assignee: 김은향
priority: medium
tags: []
type: dev
aliases:
- tsk-mzjr8a-1769414529383
start_date: '2026-02-01'
due: '2026-02-01'
start_time: '16:00'
end_time: '17:00'
notes: "# Recovery - ILOS 데이터 레이어 구현\n\n## 설명\n\nRecovery Routine + Reset을 **ILOS\
  \ 온톨로지 기반**으로 프로덕션에 통합하는 데이터 레이어 구현.\n\n**핵심 설계 결정**:\n\n- ✅ 하나의 Episode로 통합 (Recovery\
  \ Routine + Reset을 단일 세션으로)\n- ✅ Full ILOS 적용 (Episode + Event 분리)\n- ✅ OutcomeMeasurement\
  \ 자동 연결 (2시간 내 MealDiary 자동 링크)\n\n**ILOS 매핑 구조**:\n\n```\nRecoveryEpisode (Episode)\n\
  ├── uuid, episodeType: 'urge_response'\n├── startedAt, completedAt\n├── caseType:\
  \ 'home' | 'mobile'\n├── actionType: 'breathing' | 'water_cup'\n├── cravingBefore,\
  \ cravingAfter\n│\n├── RecoveryEvent[] (Events)\n│   ├── emotion: {emotion: 'anxious',\
  \ emoji: '\U0001F630'}\n│   ├── trigger: {triggers: ['alone', 'stress'], customText:\
  \ '...'}\n│   ├── thought: {thoughtText: '...'}\n│   ├── craving_before: {level:\
  \ 6.5}\n│   └── craving_after: {level: 3.0}\n│\n└── MealDiary (OutcomeMeasurement)\n\
  \    └── recoveryEpisodeUuid → 자동 연결\n```\n\n---\n\n## Tech Spec\n\n### Architecture\
  \ Compliance\n\n- **Parent Project**: prj-mzjr8a (KkokKkok App v1.0.29 - Release)\n\
  - **Target**: sosi (Flutter)\n- **Pattern**: Local-First + Outbox Pattern (기존 아키텍처\
  \ 준수)\n- **State**: flutter_bloc + hydrated_bloc\n\n### File Structure - 신규 생성\n\
  \n```\nlib/data/models/\n├── recovery_episode.dart        # Episode 모델 (freezed)\n\
  ├── recovery_episode.freezed.dart\n├── recovery_episode.g.dart\n├── recovery_event.dart\
  \          # Event 모델 (freezed)\n├── recovery_event.freezed.dart\n└── recovery_event.g.dart\n\
  \nlib/data/local/drift/tables/\n├── recovery_episode_table.dart  # Drift 테이블 정의\n\
  └── recovery_event_table.dart\n\nlib/data/local/\n├── recovery_episode_dao.dart\
  \    # Episode CRUD\n└── recovery_event_dao.dart      # Event CRUD\n\nlib/data/repositories/\n\
  └── recovery_repository.dart     # 비즈니스 로직 통합\n\nlib/data/remote/\n├── firestore_recovery_episode_data_source.dart\n\
  └── firestore_recovery_event_data_source.dart\n\nlib/data/local/database/migrations/\n\
  └── migration_v11.dart           # DB 마이그레이션\n```\n\n### File Structure - 수정\n\n\
  ```\nlib/data/models/outbox_operation.dart\n  → OutboxEntity enum에 recoveryEpisode,\
  \ recoveryEvent 추가\n\nlib/data/local/drift/app_database.dart\n  → 테이블 등록 + schemaVersion\
  \ 11\n\nlib/data/local/database_helper.dart\n  → DAO 인스턴스 추가\n\nlib/data/services/operation_router.dart\n\
  \  → Recovery 핸들러 추가\n\nlib/data/services/sync_manager_impl.dart\n  → Pull 타겟 추가\n\
  \nlib/data/local/drift/tables/meal_diary_table.dart\n  → recoveryEpisodeUuid 컬럼\
  \ 추가\n\nlib/data/local/meal_diary_dao.dart\n  → 자동연결 메서드 추가\n```\n\n### Implementation\
  \ Details\n\n1\\. RecoveryEpisode 모델\n\n```dart\n@freezed\nclass RecoveryEpisode\
  \ with _$RecoveryEpisode {\n  const factory RecoveryEpisode({\n    int? id,\n  \
  \  required String uuid,\n    required String episodeType,    // 'urge_response'\
  \ | 'risk_window'\n    required String caseType,       // 'home' | 'mobile'\n  \
  \  required DateTime startedAt,\n    DateTime? completedAt,\n    String? actionType,\
  \             // 'breathing' | 'water_cup' | 'awareness'\n    int? actionDurationSec,\n\
  \    @Default(false) bool actionCompleted,\n    double? cravingBefore,\n    double?\
  \ cravingAfter,\n    String? triggerMealDiaryUuid,\n    required DateTime updatedAt,\n\
  \    DateTime? deletedAt,\n  }) = _RecoveryEpisode;\n}\n```\n\n2\\. RecoveryEvent\
  \ 모델\n\n```dart\n@freezed\nclass RecoveryEvent with _$RecoveryEvent {\n  const factory\
  \ RecoveryEvent({\n    int? id,\n    required String uuid,\n    required String\
  \ episodeUuid,    // FK\n    required String eventType,      // 'emotion' | 'trigger'\
  \ | 'thought' | 'craving_before' | 'craving_after'\n    required String payloadJson,\
  \    // JSON encoded payload\n    required DateTime recordedAt,\n    required DateTime\
  \ updatedAt,\n    DateTime? deletedAt,\n  }) = _RecoveryEvent;\n}\n```\n\n3\\. Drift\
  \ 테이블\n\n- `RecoveryEpisodeTable`: uuid(PK), episodeType, caseType, startedAt, completedAt,\
  \ actionType, actionDurationSec, actionCompleted, cravingBefore, cravingAfter, triggerMealDiaryUuid,\
  \ updatedAt, deletedAt\n- `RecoveryEventTable`: uuid(PK), episodeUuid(FK), eventType,\
  \ payloadJson, recordedAt, updatedAt, deletedAt\n\n4\\. RecoveryRepository 핵심 메서드\n\
  \n```dart\nFuture<RecoveryEpisode> startSession(String caseType);\nFuture<void>\
  \ recordEmotion(String episodeUuid, RecoveryEmotion emotion);\nFuture<void> recordTriggers(String\
  \ episodeUuid, List<String> triggers, String? custom);\nFuture<void> recordThought(String\
  \ episodeUuid, String text);\nFuture<void> recordCraving(String episodeUuid, double\
  \ level, String phase);\nFuture<void> completeSession(String episodeUuid, {...});\n\
  Future<void> linkPendingMealOutcome(String mealDiaryUuid);\n```\n\n5\\. Outcome\
  \ 자동연결 로직\n\n- MealDiary 저장 시 2시간 내 완료된 RecoveryEpisode 검색\n- 발견 시 `recoveryEpisodeUuid`\
  \ 필드 자동 설정\n\n### Edge Cases\n\n1. **앱 강제 종료 시**: Episode가 completedAt 없이 남음 → 24시간\
  \ 이상 미완료 Episode는 자동 정리 대상\n2. **동시 세션 방지**: startSession() 호출 시 진행 중인 Episode 있으면\
  \ 자동 완료 처리\n3. **동기화 충돌**: updatedAt 기준 last-write-wins (기존 패턴 준수)\n4. **Firestore\
  \ 쿼리 최적화**: episodeUuid 인덱스 필수\n\n### Firestore Collection 구조\n\n```\nusers/{uid}/\n\
  ├── recovery_episodes/{uuid}\n│   ├── uuid, episodeType, caseType\n│   ├── startedAt,\
  \ completedAt\n│   ├── actionType, actionDurationSec, actionCompleted\n│   ├── cravingBefore,\
  \ cravingAfter\n│   └── updatedAt, deletedAt, triggerMealDiaryUuid\n│\n├── recovery_events/{uuid}\n\
  │   ├── uuid, episodeUuid\n│   ├── eventType, payloadJson\n│   ├── recordedAt\n\
  │   └── updatedAt, deletedAt\n│\n└── meal_diaries/{uuid}\n    └── recoveryEpisodeUuid\
  \ (nullable)\n```\n\n---\n\n## 체크리스트\n\n### Phase 1: 데이터 모델 + DB\n\n- [ ] `lib/data/models/recovery_episode.dart`\
  \ - freezed 모델 생성\n\n- [ ] `lib/data/models/recovery_event.dart` - freezed 모델 생성\n\
  \n- [ ] `lib/data/local/drift/tables/recovery_episode_table.dart` - Drift 테이블\n\n\
  - [ ] `lib/data/local/drift/tables/recovery_event_table.dart` - Drift 테이블\n\n- [\
  \ ] `lib/data/local/database/migrations/migration_v11.dart` - 마이그레이션\n\n- [ ] `lib/data/local/drift/app_database.dart`\
  \ - 테이블 등록 + schemaVersion 11\n\n### Phase 2: DAO\n\n- [ ] `lib/data/local/recovery_episode_dao.dart`\
  \ - BaseDao 확장\n\n- [ ] `lib/data/local/recovery_event_dao.dart` - BaseDao 확장\n\n\
  - [ ] `lib/data/local/database_helper.dart` - DAO 인스턴스 추가\n\n### Phase 3: Repository\n\
  \n- [ ] `lib/data/repositories/recovery_repository.dart` - 비즈니스 로직\n\n### Phase\
  \ 4: 동기화\n\n- [ ] `lib/data/models/outbox_operation.dart` - enum 확장\n\n- [ ] `lib/data/remote/firestore_recovery_episode_data_source.dart`\n\
  \n- [ ] `lib/data/remote/firestore_recovery_event_data_source.dart`\n\n- [ ] `lib/data/services/operation_router.dart`\
  \ - 핸들러 추가\n\n- [ ] `lib/data/services/sync_manager_impl.dart` - Pull 타겟 추가\n\n\
  ### Phase 5: Outcome 연결\n\n- [ ] `lib/data/local/drift/tables/meal_diary_table.dart`\
  \ - recoveryEpisodeUuid 추가\n\n- [ ] `lib/data/local/meal_diary_dao.dart` - linkRecoveryEpisode()\
  \ 메서드\n\n### Phase 6: 검증\n\n- [ ] `flutter pub run build_runner build --delete-conflicting-outputs`\n\
  \n- [ ] `flutter analyze` 통과\n\n- [ ] 마이그레이션 테스트 (기존 데이터 보존 확인)\n\n---\n\n## 부록\
  \ A: 상세 스키마 정의 (Codex 이슈 해결)\n\n### A1. Enum 값 정의\n\n```dart\n/// Episode 타입\nenum\
  \ RecoveryEpisodeType {\n  urgeResponse,   // 충동 대응 (즉각적)\n  riskWindow,     //\
  \ 위험 시간대 (예방적)\n}\n\n/// Case 타입 (UI 흐름)\nenum RecoveryCaseType {\n  home,    //\
  \ 집에서 (전체 플로우)\n  mobile,  // 이동 중 (간소화 플로우)\n}\n\n/// Action 타입\nenum RecoveryActionType\
  \ {\n  breathing,  // 호흡 운동\n  waterCup,   // 물 한 컵\n  awareness,  // 알아차림 (생각 기록만)\n\
  }\n\n/// Event 타입\nenum RecoveryEventType {\n  emotion,        // 감정 선택\n  trigger,\
  \        // 트리거 선택\n  thought,        // 생각 기록\n  cravingBefore,  // 행동 전 갈망\n \
  \ cravingAfter,   // 행동 후 갈망\n}\n```\n\n### A2. Drift 테이블 상세 스키마\n\n```dart\n//\
  \ recovery_episode_table.dart\n@DataClassName('RecoveryEpisodeEntry')\nclass RecoveryEpisodeTable\
  \ extends Table {\n  @override\n  String get tableName => 'recovery_episode';\n\n\
  \  IntColumn get id => integer().autoIncrement()();\n  TextColumn get uuid => text().unique()();\
  \                    // NOT NULL, UNIQUE\n  TextColumn get episodeType => text()();\
  \                      // NOT NULL\n  TextColumn get caseType => text()();     \
  \                    // NOT NULL\n  DateTimeColumn get startedAt => dateTime()();\
  \                // NOT NULL\n  DateTimeColumn get completedAt => dateTime().nullable()();\
  \   // nullable\n  TextColumn get actionType => text().nullable()();           \
  \ // nullable\n  IntColumn get actionDurationSec => integer().nullable()();   //\
  \ nullable\n  BoolColumn get actionCompleted => boolean().withDefault(const Constant(false))();\n\
  \  RealColumn get cravingBefore => real().nullable()();         // nullable (0.0~10.0)\n\
  \  RealColumn get cravingAfter => real().nullable()();          // nullable (0.0~10.0)\n\
  \  TextColumn get triggerMealDiaryUuid => text().nullable()();  // nullable FK\n\
  \  DateTimeColumn get updatedAt => dateTime()();                // NOT NULL (sync용)\n\
  \  DateTimeColumn get deletedAt => dateTime().nullable()();     // soft-delete\n\
  }\n\n// recovery_event_table.dart\n@DataClassName('RecoveryEventEntry')\nclass RecoveryEventTable\
  \ extends Table {\n  @override\n  String get tableName => 'recovery_event';\n\n\
  \  IntColumn get id => integer().autoIncrement()();\n  TextColumn get uuid => text().unique()();\
  \                    // NOT NULL, UNIQUE\n  TextColumn get episodeUuid => text()();\
  \                      // NOT NULL, FK (no cascade)\n  TextColumn get eventType\
  \ => text()();                        // NOT NULL\n  TextColumn get payloadJson\
  \ => text()();                      // NOT NULL, JSON string\n  DateTimeColumn get\
  \ recordedAt => dateTime()();               // NOT NULL\n  DateTimeColumn get updatedAt\
  \ => dateTime()();                // NOT NULL (sync용)\n  DateTimeColumn get deletedAt\
  \ => dateTime().nullable()();     // soft-delete\n}\n```\n\n**인덱스 정의** (migration_v11.dart에서\
  \ 생성):\n\n- `recovery_episode`: `uuid` (UNIQUE), `updatedAt` (동기화 쿼리)\n- `recovery_event`:\
  \ `uuid` (UNIQUE), `episodeUuid` (FK 조회), `episodeUuid + updatedAt` (복합)\n\n**FK\
  \ 정책**: No CASCADE - Episode 삭제 시 Event도 명시적 soft-delete 처리\n\n### A3. Outbox 플로우\
  \ 상세\n\n```dart\n// outbox_operation.dart 수정\nenum OutboxEntity {\n  diary,\n  routine,\n\
  \  routineTemplate,\n  profile,\n  routineMealDiaryLink,\n  routineInstance,\n \
  \ weeklyReport,\n  chatMessage,\n  recoveryEpisode,  // \U0001F195 추가\n  recoveryEvent,\
  \    // \U0001F195 추가\n}\n\n// OutboxAction은 기존 그대로 사용: create, update, delete\n\
  ```\n\n**Outbox Payload 스키마**:\n\n| Entity | Action | Payload |\n| --- | --- | ---\
  \ |\n| recoveryEpisode | create/update | 전체 필드 JSON |\n| recoveryEpisode | delete\
  \ | `{ \"uuid\": \"...\" }` |\n| recoveryEvent | create/update | 전체 필드 JSON |\n\
  | recoveryEvent | delete | `{ \"uuid\": \"...\" }` |\n\n**Delete 정책**: Soft-delete\
  \ (deletedAt 설정) - 기존 패턴 준수\n\n### A4. Sync/Pull 타겟 상세\n\n```dart\n// sync_manager_impl.dart\
  \ 추가\n_SyncPullTarget(\n  name: 'recovery_episode',\n  fetchUpdated: (s) => recoveryEpisodeDataSource.fetchUpdatedEpisodes(s),\n\
  \  upsertLocal: (e) async {\n    await recoveryEpisodeDao.upsert(e as RecoveryEpisode);\n\
  \  },\n),\n_SyncPullTarget(\n  name: 'recovery_event',\n  fetchUpdated: (s) => recoveryEventDataSource.fetchUpdatedEvents(s),\n\
  \  upsertLocal: (e) async {\n    await recoveryEventDao.upsert(e as RecoveryEvent);\n\
  \  },\n),\n```\n\n**동기화 순서**:\n\n1. recovery_episode (먼저)\n2. recovery_event (나중\
  \ - FK 의존)\n\n**고아 Event 처리**:\n\n- Pull 시 episodeUuid가 로컬에 없으면 → Event 저장 건너뜀 (다음\
  \ Sync 시 Episode가 올 수 있음)\n- 또는 orphanedEpisodeUuids 리스트 유지 후 Episode Pull 완료 후\
  \ 재처리\n\n**충돌 정책**: Last-write-wins (updatedAt 기준)\n\n### A5. 마이그레이션 상세 (v11)\n\n\
  ```dart\n// database_config.dart\nstatic const int version = 11; // v11 recovery_episode\
  \ + recovery_event\n\n// migration_v11.dart\nFuture<void> migrateToV11(Database\
  \ db) async {\n  await db.transaction((txn) async {\n    // 1. recovery_episode\
  \ 테이블 생성\n    await txn.execute('''\n      CREATE TABLE IF NOT EXISTS recovery_episode\
  \ (\n        id INTEGER PRIMARY KEY AUTOINCREMENT,\n        uuid TEXT NOT NULL UNIQUE,\n\
  \        episodeType TEXT NOT NULL,\n        caseType TEXT NOT NULL,\n        startedAt\
  \ TEXT NOT NULL,\n        completedAt TEXT,\n        actionType TEXT,\n        actionDurationSec\
  \ INTEGER,\n        actionCompleted INTEGER NOT NULL DEFAULT 0,\n        cravingBefore\
  \ REAL,\n        cravingAfter REAL,\n        triggerMealDiaryUuid TEXT,\n      \
  \  updatedAt TEXT NOT NULL,\n        deletedAt TEXT\n      )\n    ''');\n\n    //\
  \ 2. recovery_event 테이블 생성\n    await txn.execute('''\n      CREATE TABLE IF NOT\
  \ EXISTS recovery_event (\n        id INTEGER PRIMARY KEY AUTOINCREMENT,\n     \
  \   uuid TEXT NOT NULL UNIQUE,\n        episodeUuid TEXT NOT NULL,\n        eventType\
  \ TEXT NOT NULL,\n        payloadJson TEXT NOT NULL,\n        recordedAt TEXT NOT\
  \ NULL,\n        updatedAt TEXT NOT NULL,\n        deletedAt TEXT\n      )\n   \
  \ ''');\n\n    // 3. 인덱스 생성\n    await txn.execute('CREATE INDEX IF NOT EXISTS idx_episode_updated\
  \ ON recovery_episode(updatedAt)');\n    await txn.execute('CREATE INDEX IF NOT\
  \ EXISTS idx_event_episode ON recovery_event(episodeUuid)');\n    await txn.execute('CREATE\
  \ INDEX IF NOT EXISTS idx_event_updated ON recovery_event(updatedAt)');\n\n    //\
  \ 4. meal_diary에 recoveryEpisodeUuid 컬럼 추가\n    final columns = await txn.rawQuery(\"\
  PRAGMA table_info('meal_diary')\");\n    final hasColumn = columns.any((c) => c['name']\
  \ == 'recoveryEpisodeUuid');\n    if (!hasColumn) {\n      await txn.execute('ALTER\
  \ TABLE meal_diary ADD COLUMN recoveryEpisodeUuid TEXT');\n    }\n\n    // 5. 검증\n\
  \    await _validateMigration(txn);\n  });\n}\n```\n\n### A6. MealDiary 자동연결 상세\n\
  \n**기준 타임스탬프**: `Episode.completedAt`\n\n**로직**:\n\n```dart\n// meal_diary_dao.dart\n\
  Future<void> linkRecoveryEpisodeIfNeeded(String mealDiaryUuid, DateTime mealTime)\
  \ async {\n  // 1. mealTime 기준 2시간 이전 ~ mealTime 사이에 completedAt이 있는 Episode 검색\n\
  \  final twoHoursBefore = mealTime.subtract(Duration(hours: 2));\n\n  final episodes\
  \ = await db.query(\n    'recovery_episode',\n    where: 'completedAt IS NOT NULL\
  \ AND completedAt >= ? AND completedAt <= ? AND deletedAt IS NULL',\n    whereArgs:\
  \ [twoHoursBefore.toIso8601String(), mealTime.toIso8601String()],\n    orderBy:\
  \ 'completedAt DESC',\n    limit: 1,\n  );\n\n  if (episodes.isEmpty) return;\n\n\
  \  final episodeUuid = episodes.first['uuid'] as String;\n\n  // 2. MealDiary에 연결\
  \ (이미 연결된 경우 덮어쓰기 안 함)\n  await db.update(\n    'meal_diary',\n    {'recoveryEpisodeUuid':\
  \ episodeUuid, 'updatedAt': DateTime.now().toUtc().toIso8601String()},\n    where:\
  \ 'uuid = ? AND recoveryEpisodeUuid IS NULL',\n    whereArgs: [mealDiaryUuid],\n\
  \  );\n\n  // 3. Outbox 기록 (Firestore 동기화)\n  // ... OutboxOperation 생성\n}\n```\n\
  \n**다중 MealDiary 처리**: 가장 최근 completedAt Episode 1개만 연결\n\n**Episode 삭제 시**: MealDiary의\
  \ recoveryEpisodeUuid를 NULL로 초기화하지 않음 (히스토리 보존)\n\n**연결 시점**: MealDiary INSERT/UPDATE\
  \ 후 호출\n\n### A7. Repository API 상세\n\n```dart\nclass RecoveryRepository {\n  RecoveryRepository({\n\
  \    required this.episodeDao,\n    required this.eventDao,\n    required this.outboxDao,\n\
  \  });\n\n  final RecoveryEpisodeDao episodeDao;\n  final RecoveryEventDao eventDao;\n\
  \  final OutboxDao outboxDao;\n\n  /// 새 세션 시작 - 진행 중인 Episode 있으면 자동 완료 처리\n  Future<RecoveryEpisode>\
  \ startSession({\n    required RecoveryCaseType caseType,\n    RecoveryEpisodeType\
  \ episodeType = RecoveryEpisodeType.urgeResponse,\n  }) async {\n    // 1. 진행 중인\
  \ Episode 완료 처리\n    await _completeIncompleteEpisodes();\n\n    // 2. 새 Episode\
  \ 생성\n    final episode = RecoveryEpisode(\n      uuid: Uuid().v4(),\n      episodeType:\
  \ episodeType.name,\n      caseType: caseType.name,\n      startedAt: DateTime.now().toUtc(),\n\
  \      updatedAt: DateTime.now().toUtc(),\n    );\n\n    await episodeDao.upsert(episode);\n\
  \    await _enqueueOutbox(OutboxEntity.recoveryEpisode, OutboxAction.create, episode);\n\
  \n    return episode;\n  }\n\n  /// Event 기록 (내부적으로 Outbox 자동 생성)\n  Future<RecoveryEvent>\
  \ _recordEvent({\n    required String episodeUuid,\n    required RecoveryEventType\
  \ eventType,\n    required Map<String, dynamic> payload,\n  }) async {\n    final\
  \ event = RecoveryEvent(\n      uuid: Uuid().v4(),\n      episodeUuid: episodeUuid,\n\
  \      eventType: eventType.name,\n      payloadJson: jsonEncode(payload),\n   \
  \   recordedAt: DateTime.now().toUtc(),\n      updatedAt: DateTime.now().toUtc(),\n\
  \    );\n\n    await eventDao.upsert(event);\n    await _enqueueOutbox(OutboxEntity.recoveryEvent,\
  \ OutboxAction.create, event);\n\n    return event;\n  }\n\n  /// 감정 기록\n  Future<void>\
  \ recordEmotion(String episodeUuid, String emotion, String emoji) =>\n    _recordEvent(\n\
  \      episodeUuid: episodeUuid,\n      eventType: RecoveryEventType.emotion,\n\
  \      payload: {'emotion': emotion, 'emoji': emoji},\n    );\n\n  /// 트리거 기록\n\
  \  Future<void> recordTriggers(String episodeUuid, List<String> triggers, String?\
  \ customText) =>\n    _recordEvent(\n      episodeUuid: episodeUuid,\n      eventType:\
  \ RecoveryEventType.trigger,\n      payload: {'triggers': triggers, 'customText':\
  \ customText},\n    );\n\n  /// 생각 기록\n  Future<void> recordThought(String episodeUuid,\
  \ String text) =>\n    _recordEvent(\n      episodeUuid: episodeUuid,\n      eventType:\
  \ RecoveryEventType.thought,\n      payload: {'thoughtText': text},\n    );\n\n\
  \  /// 갈망 레벨 기록\n  Future<void> recordCraving(String episodeUuid, double level,\
  \ String phase) async {\n    final eventType = phase == 'before'\n      ? RecoveryEventType.cravingBefore\n\
  \      : RecoveryEventType.cravingAfter;\n\n    await _recordEvent(\n      episodeUuid:\
  \ episodeUuid,\n      eventType: eventType,\n      payload: {'level': level},\n\
  \    );\n\n    // Episode에도 denormalized 저장 (쿼리 편의)\n    final field = phase ==\
  \ 'before' ? 'cravingBefore' : 'cravingAfter';\n    await episodeDao.updateField(episodeUuid,\
  \ field, level);\n  }\n\n  /// 세션 완료\n  Future<void> completeSession(\n    String\
  \ episodeUuid, {\n    RecoveryActionType? actionType,\n    int? actionDurationSec,\n\
  \    bool actionCompleted = false,\n  }) async {\n    await episodeDao.complete(\n\
  \      episodeUuid,\n      actionType: actionType?.name,\n      actionDurationSec:\
  \ actionDurationSec,\n      actionCompleted: actionCompleted,\n    );\n\n    //\
  \ Outbox update\n    final episode = await episodeDao.findByUuid(episodeUuid);\n\
  \    if (episode != null) {\n      await _enqueueOutbox(OutboxEntity.recoveryEpisode,\
  \ OutboxAction.update, episode);\n    }\n  }\n\n  /// 최근 Episode 조회 (통계용)\n  Future<List<RecoveryEpisode>>\
  \ getRecentEpisodes({int days = 7}) =>\n    episodeDao.fetchRecent(days: days);\n\
  \n  /// Episode에 연결된 Event 조회\n  Future<List<RecoveryEvent>> getEventsForEpisode(String\
  \ episodeUuid) =>\n    eventDao.fetchByEpisodeUuid(episodeUuid);\n}\n```\n\n###\
  \ A8. Freezed 직렬화 상세\n\n```dart\n// recovery_episode.dart\n@freezed\nclass RecoveryEpisode\
  \ with _$RecoveryEpisode {\n  const factory RecoveryEpisode({\n    int? id,\n  \
  \  required String uuid,\n    required String episodeType,\n    required String\
  \ caseType,\n    required DateTime startedAt,\n    DateTime? completedAt,\n    String?\
  \ actionType,\n    int? actionDurationSec,\n    @Default(false) bool actionCompleted,\n\
  \    double? cravingBefore,\n    double? cravingAfter,\n    String? triggerMealDiaryUuid,\n\
  \    required DateTime updatedAt,\n    DateTime? deletedAt,\n  }) = _RecoveryEpisode;\n\
  \n  /// JSON 직렬화 (Firestore/Outbox용)\n  factory RecoveryEpisode.fromJson(Map<String,\
  \ dynamic> json) =>\n      _$RecoveryEpisodeFromJson(json);\n}\n\n// recovery_episode.g.dart\
  \ (build_runner 생성)\n// DateTime은 ISO8601 String으로 자동 변환\n```\n\n**DAO toMap/fromEntry\
  \ 변환**:\n\n```dart\nMap<String, dynamic> toMap(RecoveryEpisode model) => {\n  'id':\
  \ model.id,\n  'uuid': model.uuid,\n  'episodeType': model.episodeType,\n  'caseType':\
  \ model.caseType,\n  'startedAt': model.startedAt.toIso8601String(),\n  'completedAt':\
  \ model.completedAt?.toIso8601String(),\n  // ... 나머지 필드\n  'updatedAt': model.updatedAt.toIso8601String(),\n\
  \  'deletedAt': model.deletedAt?.toIso8601String(),\n};\n\nRecoveryEpisode fromEntry(RecoveryEpisodeEntry\
  \ e) => RecoveryEpisode(\n  id: e.id,\n  uuid: e.uuid,\n  episodeType: e.episodeType,\n\
  \  caseType: e.caseType,\n  startedAt: e.startedAt,\n  completedAt: e.completedAt,\n\
  \  // ... Drift는 DateTime 자동 처리\n  updatedAt: e.updatedAt,\n  deletedAt: e.deletedAt,\n\
  );\n```\n\n---\n\n## 참고\n\n- **BaseDao 패턴**: `lib/data/local/base_dao.dart`\n- **Drift\
  \ 마이그레이션 예시**: `lib/data/local/database/migrations/migration_v10.dart`\n- **OutboxEntity\
  \ 예시**: `lib/data/models/outbox_operation.dart`\n- **OperationRouter 예시**: `lib/data/services/operation_router.dart`\n\
  - **SyncManager 예시**: `lib/data/services/sync_manager_impl.dart`\n- **DatabaseConfig**:\
  \ `lib/data/local/database_config.dart` (현재 v10)\n\n---\n\n## 짧은 회고"
---
# Recovery - ILOS 데이터 레이어 구현

## 설명

Recovery Routine + Reset을 **ILOS 온톨로지 기반**으로 프로덕션에 통합하는 데이터 레이어 구현.

**핵심 설계 결정**:
- ✅ 하나의 Episode로 통합 (Recovery Routine + Reset을 단일 세션으로)
- ✅ Full ILOS 적용 (Episode + Event 분리)
- ✅ OutcomeMeasurement 자동 연결 (2시간 내 MealDiary 자동 링크)

**ILOS 매핑 구조**:
```
RecoveryEpisode (Episode)
├── uuid, episodeType: 'urge_response'
├── startedAt, completedAt
├── caseType: 'home' | 'mobile'
├── actionType: 'breathing' | 'water_cup'
├── cravingBefore, cravingAfter
│
├── RecoveryEvent[] (Events)
│   ├── emotion: {emotion: 'anxious', emoji: '😰'}
│   ├── trigger: {triggers: ['alone', 'stress'], customText: '...'}
│   ├── thought: {thoughtText: '...'}
│   ├── craving_before: {level: 6.5}
│   └── craving_after: {level: 3.0}
│
└── MealDiary (OutcomeMeasurement)
    └── recoveryEpisodeUuid → 자동 연결
```

---

## Tech Spec

### Architecture Compliance
- **Parent Project**: prj-mzjr8a (KkokKkok App v1.0.29 - Release)
- **Target**: sosi (Flutter)
- **Pattern**: Local-First + Outbox Pattern (기존 아키텍처 준수)
- **State**: flutter_bloc + hydrated_bloc

### File Structure - 신규 생성

```
lib/data/models/
├── recovery_episode.dart        # Episode 모델 (freezed)
├── recovery_episode.freezed.dart
├── recovery_episode.g.dart
├── recovery_event.dart          # Event 모델 (freezed)
├── recovery_event.freezed.dart
└── recovery_event.g.dart

lib/data/local/drift/tables/
├── recovery_episode_table.dart  # Drift 테이블 정의
└── recovery_event_table.dart

lib/data/local/
├── recovery_episode_dao.dart    # Episode CRUD
└── recovery_event_dao.dart      # Event CRUD

lib/data/repositories/
└── recovery_repository.dart     # 비즈니스 로직 통합

lib/data/remote/
├── firestore_recovery_episode_data_source.dart
└── firestore_recovery_event_data_source.dart

lib/data/local/database/migrations/
└── migration_v11.dart           # DB 마이그레이션
```

### File Structure - 수정

```
lib/data/models/outbox_operation.dart
  → OutboxEntity enum에 recoveryEpisode, recoveryEvent 추가

lib/data/local/drift/app_database.dart
  → 테이블 등록 + schemaVersion 11

lib/data/local/database_helper.dart
  → DAO 인스턴스 추가

lib/data/services/operation_router.dart
  → Recovery 핸들러 추가

lib/data/services/sync_manager_impl.dart
  → Pull 타겟 추가

lib/data/local/drift/tables/meal_diary_table.dart
  → recoveryEpisodeUuid 컬럼 추가

lib/data/local/meal_diary_dao.dart
  → 자동연결 메서드 추가
```

### Implementation Details

#### 1. RecoveryEpisode 모델
```dart
@freezed
class RecoveryEpisode with _$RecoveryEpisode {
  const factory RecoveryEpisode({
    int? id,
    required String uuid,
    required String episodeType,    // 'urge_response' | 'risk_window'
    required String caseType,       // 'home' | 'mobile'
    required DateTime startedAt,
    DateTime? completedAt,
    String? actionType,             // 'breathing' | 'water_cup' | 'awareness'
    int? actionDurationSec,
    @Default(false) bool actionCompleted,
    double? cravingBefore,
    double? cravingAfter,
    String? triggerMealDiaryUuid,
    required DateTime updatedAt,
    DateTime? deletedAt,
  }) = _RecoveryEpisode;
}
```

#### 2. RecoveryEvent 모델
```dart
@freezed
class RecoveryEvent with _$RecoveryEvent {
  const factory RecoveryEvent({
    int? id,
    required String uuid,
    required String episodeUuid,    // FK
    required String eventType,      // 'emotion' | 'trigger' | 'thought' | 'craving_before' | 'craving_after'
    required String payloadJson,    // JSON encoded payload
    required DateTime recordedAt,
    required DateTime updatedAt,
    DateTime? deletedAt,
  }) = _RecoveryEvent;
}
```

#### 3. Drift 테이블
- `RecoveryEpisodeTable`: uuid(PK), episodeType, caseType, startedAt, completedAt, actionType, actionDurationSec, actionCompleted, cravingBefore, cravingAfter, triggerMealDiaryUuid, updatedAt, deletedAt
- `RecoveryEventTable`: uuid(PK), episodeUuid(FK), eventType, payloadJson, recordedAt, updatedAt, deletedAt

#### 4. RecoveryRepository 핵심 메서드
```dart
Future<RecoveryEpisode> startSession(String caseType);
Future<void> recordEmotion(String episodeUuid, RecoveryEmotion emotion);
Future<void> recordTriggers(String episodeUuid, List<String> triggers, String? custom);
Future<void> recordThought(String episodeUuid, String text);
Future<void> recordCraving(String episodeUuid, double level, String phase);
Future<void> completeSession(String episodeUuid, {...});
Future<void> linkPendingMealOutcome(String mealDiaryUuid);
```

#### 5. Outcome 자동연결 로직
- MealDiary 저장 시 2시간 내 완료된 RecoveryEpisode 검색
- 발견 시 `recoveryEpisodeUuid` 필드 자동 설정

### Edge Cases

1. **앱 강제 종료 시**: Episode가 completedAt 없이 남음 → 24시간 이상 미완료 Episode는 자동 정리 대상
2. **동시 세션 방지**: startSession() 호출 시 진행 중인 Episode 있으면 자동 완료 처리
3. **동기화 충돌**: updatedAt 기준 last-write-wins (기존 패턴 준수)
4. **Firestore 쿼리 최적화**: episodeUuid 인덱스 필수

### Firestore Collection 구조

```
users/{uid}/
├── recovery_episodes/{uuid}
│   ├── uuid, episodeType, caseType
│   ├── startedAt, completedAt
│   ├── actionType, actionDurationSec, actionCompleted
│   ├── cravingBefore, cravingAfter
│   └── updatedAt, deletedAt, triggerMealDiaryUuid
│
├── recovery_events/{uuid}
│   ├── uuid, episodeUuid
│   ├── eventType, payloadJson
│   ├── recordedAt
│   └── updatedAt, deletedAt
│
└── meal_diaries/{uuid}
    └── recoveryEpisodeUuid (nullable)
```

---

## 체크리스트

### Phase 1: 데이터 모델 + DB
- [ ] `lib/data/models/recovery_episode.dart` - freezed 모델 생성
- [ ] `lib/data/models/recovery_event.dart` - freezed 모델 생성
- [ ] `lib/data/local/drift/tables/recovery_episode_table.dart` - Drift 테이블
- [ ] `lib/data/local/drift/tables/recovery_event_table.dart` - Drift 테이블
- [ ] `lib/data/local/database/migrations/migration_v11.dart` - 마이그레이션
- [ ] `lib/data/local/drift/app_database.dart` - 테이블 등록 + schemaVersion 11

### Phase 2: DAO
- [ ] `lib/data/local/recovery_episode_dao.dart` - BaseDao 확장
- [ ] `lib/data/local/recovery_event_dao.dart` - BaseDao 확장
- [ ] `lib/data/local/database_helper.dart` - DAO 인스턴스 추가

### Phase 3: Repository
- [ ] `lib/data/repositories/recovery_repository.dart` - 비즈니스 로직

### Phase 4: 동기화
- [ ] `lib/data/models/outbox_operation.dart` - enum 확장
- [ ] `lib/data/remote/firestore_recovery_episode_data_source.dart`
- [ ] `lib/data/remote/firestore_recovery_event_data_source.dart`
- [ ] `lib/data/services/operation_router.dart` - 핸들러 추가
- [ ] `lib/data/services/sync_manager_impl.dart` - Pull 타겟 추가

### Phase 5: Outcome 연결
- [ ] `lib/data/local/drift/tables/meal_diary_table.dart` - recoveryEpisodeUuid 추가
- [ ] `lib/data/local/meal_diary_dao.dart` - linkRecoveryEpisode() 메서드

### Phase 6: 검증
- [ ] `flutter pub run build_runner build --delete-conflicting-outputs`
- [ ] `flutter analyze` 통과
- [ ] 마이그레이션 테스트 (기존 데이터 보존 확인)

---

## 부록 A: 상세 스키마 정의 (Codex 이슈 해결)

### A1. Enum 값 정의

```dart
/// Episode 타입
enum RecoveryEpisodeType {
  urgeResponse,   // 충동 대응 (즉각적)
  riskWindow,     // 위험 시간대 (예방적)
}

/// Case 타입 (UI 흐름)
enum RecoveryCaseType {
  home,    // 집에서 (전체 플로우)
  mobile,  // 이동 중 (간소화 플로우)
}

/// Action 타입
enum RecoveryActionType {
  breathing,  // 호흡 운동
  waterCup,   // 물 한 컵
  awareness,  // 알아차림 (생각 기록만)
}

/// Event 타입
enum RecoveryEventType {
  emotion,        // 감정 선택
  trigger,        // 트리거 선택
  thought,        // 생각 기록
  cravingBefore,  // 행동 전 갈망
  cravingAfter,   // 행동 후 갈망
}
```

### A2. Drift 테이블 상세 스키마

```dart
// recovery_episode_table.dart
@DataClassName('RecoveryEpisodeEntry')
class RecoveryEpisodeTable extends Table {
  @override
  String get tableName => 'recovery_episode';

  IntColumn get id => integer().autoIncrement()();
  TextColumn get uuid => text().unique()();                    // NOT NULL, UNIQUE
  TextColumn get episodeType => text()();                      // NOT NULL
  TextColumn get caseType => text()();                         // NOT NULL
  DateTimeColumn get startedAt => dateTime()();                // NOT NULL
  DateTimeColumn get completedAt => dateTime().nullable()();   // nullable
  TextColumn get actionType => text().nullable()();            // nullable
  IntColumn get actionDurationSec => integer().nullable()();   // nullable
  BoolColumn get actionCompleted => boolean().withDefault(const Constant(false))();
  RealColumn get cravingBefore => real().nullable()();         // nullable (0.0~10.0)
  RealColumn get cravingAfter => real().nullable()();          // nullable (0.0~10.0)
  TextColumn get triggerMealDiaryUuid => text().nullable()();  // nullable FK
  DateTimeColumn get updatedAt => dateTime()();                // NOT NULL (sync용)
  DateTimeColumn get deletedAt => dateTime().nullable()();     // soft-delete
}

// recovery_event_table.dart
@DataClassName('RecoveryEventEntry')
class RecoveryEventTable extends Table {
  @override
  String get tableName => 'recovery_event';

  IntColumn get id => integer().autoIncrement()();
  TextColumn get uuid => text().unique()();                    // NOT NULL, UNIQUE
  TextColumn get episodeUuid => text()();                      // NOT NULL, FK (no cascade)
  TextColumn get eventType => text()();                        // NOT NULL
  TextColumn get payloadJson => text()();                      // NOT NULL, JSON string
  DateTimeColumn get recordedAt => dateTime()();               // NOT NULL
  DateTimeColumn get updatedAt => dateTime()();                // NOT NULL (sync용)
  DateTimeColumn get deletedAt => dateTime().nullable()();     // soft-delete
}
```

**인덱스 정의** (migration_v11.dart에서 생성):
- `recovery_episode`: `uuid` (UNIQUE), `updatedAt` (동기화 쿼리)
- `recovery_event`: `uuid` (UNIQUE), `episodeUuid` (FK 조회), `episodeUuid + updatedAt` (복합)

**FK 정책**: No CASCADE - Episode 삭제 시 Event도 명시적 soft-delete 처리

### A3. Outbox 플로우 상세

```dart
// outbox_operation.dart 수정
enum OutboxEntity {
  diary,
  routine,
  routineTemplate,
  profile,
  routineMealDiaryLink,
  routineInstance,
  weeklyReport,
  chatMessage,
  recoveryEpisode,  // 🆕 추가
  recoveryEvent,    // 🆕 추가
}

// OutboxAction은 기존 그대로 사용: create, update, delete
```

**Outbox Payload 스키마**:

| Entity | Action | Payload |
|--------|--------|---------|
| recoveryEpisode | create/update | 전체 필드 JSON |
| recoveryEpisode | delete | `{ "uuid": "..." }` |
| recoveryEvent | create/update | 전체 필드 JSON |
| recoveryEvent | delete | `{ "uuid": "..." }` |

**Delete 정책**: Soft-delete (deletedAt 설정) - 기존 패턴 준수

### A4. Sync/Pull 타겟 상세

```dart
// sync_manager_impl.dart 추가
_SyncPullTarget(
  name: 'recovery_episode',
  fetchUpdated: (s) => recoveryEpisodeDataSource.fetchUpdatedEpisodes(s),
  upsertLocal: (e) async {
    await recoveryEpisodeDao.upsert(e as RecoveryEpisode);
  },
),
_SyncPullTarget(
  name: 'recovery_event',
  fetchUpdated: (s) => recoveryEventDataSource.fetchUpdatedEvents(s),
  upsertLocal: (e) async {
    await recoveryEventDao.upsert(e as RecoveryEvent);
  },
),
```

**동기화 순서**:
1. recovery_episode (먼저)
2. recovery_event (나중 - FK 의존)

**고아 Event 처리**:
- Pull 시 episodeUuid가 로컬에 없으면 → Event 저장 건너뜀 (다음 Sync 시 Episode가 올 수 있음)
- 또는 orphanedEpisodeUuids 리스트 유지 후 Episode Pull 완료 후 재처리

**충돌 정책**: Last-write-wins (updatedAt 기준)

### A5. 마이그레이션 상세 (v11)

```dart
// database_config.dart
static const int version = 11; // v11 recovery_episode + recovery_event

// migration_v11.dart
Future<void> migrateToV11(Database db) async {
  await db.transaction((txn) async {
    // 1. recovery_episode 테이블 생성
    await txn.execute('''
      CREATE TABLE IF NOT EXISTS recovery_episode (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        uuid TEXT NOT NULL UNIQUE,
        episodeType TEXT NOT NULL,
        caseType TEXT NOT NULL,
        startedAt TEXT NOT NULL,
        completedAt TEXT,
        actionType TEXT,
        actionDurationSec INTEGER,
        actionCompleted INTEGER NOT NULL DEFAULT 0,
        cravingBefore REAL,
        cravingAfter REAL,
        triggerMealDiaryUuid TEXT,
        updatedAt TEXT NOT NULL,
        deletedAt TEXT
      )
    ''');

    // 2. recovery_event 테이블 생성
    await txn.execute('''
      CREATE TABLE IF NOT EXISTS recovery_event (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        uuid TEXT NOT NULL UNIQUE,
        episodeUuid TEXT NOT NULL,
        eventType TEXT NOT NULL,
        payloadJson TEXT NOT NULL,
        recordedAt TEXT NOT NULL,
        updatedAt TEXT NOT NULL,
        deletedAt TEXT
      )
    ''');

    // 3. 인덱스 생성
    await txn.execute('CREATE INDEX IF NOT EXISTS idx_episode_updated ON recovery_episode(updatedAt)');
    await txn.execute('CREATE INDEX IF NOT EXISTS idx_event_episode ON recovery_event(episodeUuid)');
    await txn.execute('CREATE INDEX IF NOT EXISTS idx_event_updated ON recovery_event(updatedAt)');

    // 4. meal_diary에 recoveryEpisodeUuid 컬럼 추가
    final columns = await txn.rawQuery("PRAGMA table_info('meal_diary')");
    final hasColumn = columns.any((c) => c['name'] == 'recoveryEpisodeUuid');
    if (!hasColumn) {
      await txn.execute('ALTER TABLE meal_diary ADD COLUMN recoveryEpisodeUuid TEXT');
    }

    // 5. 검증
    await _validateMigration(txn);
  });
}
```

### A6. MealDiary 자동연결 상세

**기준 타임스탬프**: `Episode.completedAt`

**로직**:
```dart
// meal_diary_dao.dart
Future<void> linkRecoveryEpisodeIfNeeded(String mealDiaryUuid, DateTime mealTime) async {
  // 1. mealTime 기준 2시간 이전 ~ mealTime 사이에 completedAt이 있는 Episode 검색
  final twoHoursBefore = mealTime.subtract(Duration(hours: 2));

  final episodes = await db.query(
    'recovery_episode',
    where: 'completedAt IS NOT NULL AND completedAt >= ? AND completedAt <= ? AND deletedAt IS NULL',
    whereArgs: [twoHoursBefore.toIso8601String(), mealTime.toIso8601String()],
    orderBy: 'completedAt DESC',
    limit: 1,
  );

  if (episodes.isEmpty) return;

  final episodeUuid = episodes.first['uuid'] as String;

  // 2. MealDiary에 연결 (이미 연결된 경우 덮어쓰기 안 함)
  await db.update(
    'meal_diary',
    {'recoveryEpisodeUuid': episodeUuid, 'updatedAt': DateTime.now().toUtc().toIso8601String()},
    where: 'uuid = ? AND recoveryEpisodeUuid IS NULL',
    whereArgs: [mealDiaryUuid],
  );

  // 3. Outbox 기록 (Firestore 동기화)
  // ... OutboxOperation 생성
}
```

**다중 MealDiary 처리**: 가장 최근 completedAt Episode 1개만 연결

**Episode 삭제 시**: MealDiary의 recoveryEpisodeUuid를 NULL로 초기화하지 않음 (히스토리 보존)

**연결 시점**: MealDiary INSERT/UPDATE 후 호출

### A7. Repository API 상세

```dart
class RecoveryRepository {
  RecoveryRepository({
    required this.episodeDao,
    required this.eventDao,
    required this.outboxDao,
  });

  final RecoveryEpisodeDao episodeDao;
  final RecoveryEventDao eventDao;
  final OutboxDao outboxDao;

  /// 새 세션 시작 - 진행 중인 Episode 있으면 자동 완료 처리
  Future<RecoveryEpisode> startSession({
    required RecoveryCaseType caseType,
    RecoveryEpisodeType episodeType = RecoveryEpisodeType.urgeResponse,
  }) async {
    // 1. 진행 중인 Episode 완료 처리
    await _completeIncompleteEpisodes();

    // 2. 새 Episode 생성
    final episode = RecoveryEpisode(
      uuid: Uuid().v4(),
      episodeType: episodeType.name,
      caseType: caseType.name,
      startedAt: DateTime.now().toUtc(),
      updatedAt: DateTime.now().toUtc(),
    );

    await episodeDao.upsert(episode);
    await _enqueueOutbox(OutboxEntity.recoveryEpisode, OutboxAction.create, episode);

    return episode;
  }

  /// Event 기록 (내부적으로 Outbox 자동 생성)
  Future<RecoveryEvent> _recordEvent({
    required String episodeUuid,
    required RecoveryEventType eventType,
    required Map<String, dynamic> payload,
  }) async {
    final event = RecoveryEvent(
      uuid: Uuid().v4(),
      episodeUuid: episodeUuid,
      eventType: eventType.name,
      payloadJson: jsonEncode(payload),
      recordedAt: DateTime.now().toUtc(),
      updatedAt: DateTime.now().toUtc(),
    );

    await eventDao.upsert(event);
    await _enqueueOutbox(OutboxEntity.recoveryEvent, OutboxAction.create, event);

    return event;
  }

  /// 감정 기록
  Future<void> recordEmotion(String episodeUuid, String emotion, String emoji) =>
    _recordEvent(
      episodeUuid: episodeUuid,
      eventType: RecoveryEventType.emotion,
      payload: {'emotion': emotion, 'emoji': emoji},
    );

  /// 트리거 기록
  Future<void> recordTriggers(String episodeUuid, List<String> triggers, String? customText) =>
    _recordEvent(
      episodeUuid: episodeUuid,
      eventType: RecoveryEventType.trigger,
      payload: {'triggers': triggers, 'customText': customText},
    );

  /// 생각 기록
  Future<void> recordThought(String episodeUuid, String text) =>
    _recordEvent(
      episodeUuid: episodeUuid,
      eventType: RecoveryEventType.thought,
      payload: {'thoughtText': text},
    );

  /// 갈망 레벨 기록
  Future<void> recordCraving(String episodeUuid, double level, String phase) async {
    final eventType = phase == 'before'
      ? RecoveryEventType.cravingBefore
      : RecoveryEventType.cravingAfter;

    await _recordEvent(
      episodeUuid: episodeUuid,
      eventType: eventType,
      payload: {'level': level},
    );

    // Episode에도 denormalized 저장 (쿼리 편의)
    final field = phase == 'before' ? 'cravingBefore' : 'cravingAfter';
    await episodeDao.updateField(episodeUuid, field, level);
  }

  /// 세션 완료
  Future<void> completeSession(
    String episodeUuid, {
    RecoveryActionType? actionType,
    int? actionDurationSec,
    bool actionCompleted = false,
  }) async {
    await episodeDao.complete(
      episodeUuid,
      actionType: actionType?.name,
      actionDurationSec: actionDurationSec,
      actionCompleted: actionCompleted,
    );

    // Outbox update
    final episode = await episodeDao.findByUuid(episodeUuid);
    if (episode != null) {
      await _enqueueOutbox(OutboxEntity.recoveryEpisode, OutboxAction.update, episode);
    }
  }

  /// 최근 Episode 조회 (통계용)
  Future<List<RecoveryEpisode>> getRecentEpisodes({int days = 7}) =>
    episodeDao.fetchRecent(days: days);

  /// Episode에 연결된 Event 조회
  Future<List<RecoveryEvent>> getEventsForEpisode(String episodeUuid) =>
    eventDao.fetchByEpisodeUuid(episodeUuid);
}
```

### A8. Freezed 직렬화 상세

```dart
// recovery_episode.dart
@freezed
class RecoveryEpisode with _$RecoveryEpisode {
  const factory RecoveryEpisode({
    int? id,
    required String uuid,
    required String episodeType,
    required String caseType,
    required DateTime startedAt,
    DateTime? completedAt,
    String? actionType,
    int? actionDurationSec,
    @Default(false) bool actionCompleted,
    double? cravingBefore,
    double? cravingAfter,
    String? triggerMealDiaryUuid,
    required DateTime updatedAt,
    DateTime? deletedAt,
  }) = _RecoveryEpisode;

  /// JSON 직렬화 (Firestore/Outbox용)
  factory RecoveryEpisode.fromJson(Map<String, dynamic> json) =>
      _$RecoveryEpisodeFromJson(json);
}

// recovery_episode.g.dart (build_runner 생성)
// DateTime은 ISO8601 String으로 자동 변환
```

**DAO toMap/fromEntry 변환**:
```dart
Map<String, dynamic> toMap(RecoveryEpisode model) => {
  'id': model.id,
  'uuid': model.uuid,
  'episodeType': model.episodeType,
  'caseType': model.caseType,
  'startedAt': model.startedAt.toIso8601String(),
  'completedAt': model.completedAt?.toIso8601String(),
  // ... 나머지 필드
  'updatedAt': model.updatedAt.toIso8601String(),
  'deletedAt': model.deletedAt?.toIso8601String(),
};

RecoveryEpisode fromEntry(RecoveryEpisodeEntry e) => RecoveryEpisode(
  id: e.id,
  uuid: e.uuid,
  episodeType: e.episodeType,
  caseType: e.caseType,
  startedAt: e.startedAt,
  completedAt: e.completedAt,
  // ... Drift는 DateTime 자동 처리
  updatedAt: e.updatedAt,
  deletedAt: e.deletedAt,
);
```

---

## 참고

- **BaseDao 패턴**: `lib/data/local/base_dao.dart`
- **Drift 마이그레이션 예시**: `lib/data/local/database/migrations/migration_v10.dart`
- **OutboxEntity 예시**: `lib/data/models/outbox_operation.dart`
- **OperationRouter 예시**: `lib/data/services/operation_router.dart`
- **SyncManager 예시**: `lib/data/services/sync_manager_impl.dart`
- **DatabaseConfig**: `lib/data/local/database_config.dart` (현재 v12 - Codex 확인)

---

## 부록 B: Codex 리뷰 결과 (2026-01-28)

### 🔍 발견된 이슈 6개

| # | 이슈 | 심각도 | 해결책 |
|---|------|--------|--------|
| 1 | **DB 버전 충돌** - 현재 v12인데 계획은 v11 추가 | 🔴 Critical | **v13**으로 변경 |
| 2 | **신규 설치 스키마 누락** - schema_definitions.dart 미업데이트 | 🔴 Critical | CREATE TABLE 추가 |
| 3 | **Drift 통합 누락** - app_database.dart 테이블 미등록 | 🔴 Critical | @include + schemaVersion=13 |
| 4 | **MealDiary 모델 누락** - recoveryEpisodeUuid 필드 없음 | 🟡 High | 모델+DAO+테이블 수정 |
| 5 | **DI 와이어링 누락** - main.dart에서 주입 미설정 | 🟡 High | Repository+DataSource 등록 |
| 6 | **Sync 순서 미보장** - Episode/Event 순서 없음 | 🟡 High | Episode 먼저, Event 나중 |

### 📋 수정된 구현 계획 v2

#### 핵심 변경사항
- ✅ DB 버전: v11 → **v13** (현재 v12)
- ✅ `schema_definitions.dart` 업데이트 (신규 설치 대응)
- ✅ Drift `app_database.dart` 테이블 등록 + schemaVersion=13
- ✅ `MealDiary` 모델 + DAO + Drift 테이블 수정 (`recoveryEpisodeUuid`)
- ✅ `main.dart` DI 와이어링 완전 명시
- ✅ Sync 순서 보장: Episode → Event (순차)

#### 수정된 구현 순서

| 순서 | 작업 | 파일 |
|------|------|------|
| 1 | OutboxEntity 확장 | `outbox_operation.dart` |
| 2 | **MealDiary 모델 확장** | `meal_diary.dart` (recoveryEpisodeUuid 추가) |
| 3 | RecoveryEpisode 모델 | `recovery_episode.dart` |
| 4 | RecoveryEvent 모델 | `recovery_event.dart` |
| 5 | **MealDiary Drift 테이블 수정** | `meal_diary_table.dart` |
| 6 | RecoveryEpisode 테이블 | `recovery_episode_table.dart` |
| 7 | RecoveryEvent 테이블 | `recovery_event_table.dart` |
| 8 | **마이그레이션 v13** | `migration_v13.dart` |
| 9 | **DatabaseProvider 업데이트** | `database_provider.dart` (migrateToV13 import) |
| 10 | **schema_definitions 업데이트** | `schema_definitions.dart` (신규 설치용) |
| 11 | AppDatabase 등록 | `app_database.dart` (테이블 + schemaVersion=13) |
| 12 | DatabaseConfig 업데이트 | `database_config.dart` (version=13) |
| 13 | RecoveryEpisodeDao | `recovery_episode_dao.dart` |
| 14 | RecoveryEventDao | `recovery_event_dao.dart` |
| 15 | DatabaseHelper DAO 등록 | `database_helper.dart` |
| 16 | **MealDiary DAO 확장** | `meal_diary_dao.dart` (toMap + linkRecoveryEpisode) |
| 17 | RecoveryRepository | `recovery_repository.dart` |
| 18 | Firestore DataSource (Episode) | `firestore_recovery_episode_data_source.dart` |
| 19 | Firestore DataSource (Event) | `firestore_recovery_event_data_source.dart` |
| 20 | **OperationRouter 핸들러** | `operation_router.dart` (생성자 수정 + 핸들러) |
| 21 | **SyncManager Pull 타겟** | `sync_manager_impl.dart` (생성자 수정 + 순차 Pull) |
| 22 | **main.dart DI 와이어링** | `main.dart` (Repository + DataSource 주입) |
| 23 | 검증 | build_runner + flutter analyze |

#### 추가된 수정 대상 파일

```
lib/data/models/meal_diary.dart
  → recoveryEpisodeUuid 필드 추가
  → toJson/fromJson 업데이트

lib/data/local/database_provider.dart
  → import migration_v13.dart
  → onUpgrade에서 from < 13 처리

lib/data/local/database/schema_definitions.dart
  → recovery_episode, recovery_event CREATE TABLE 추가
  → meal_diary에 recoveryEpisodeUuid 컬럼 포함

lib/main.dart
  → FirestoreRecoveryEpisodeDataSource 인스턴스 생성
  → FirestoreRecoveryEventDataSource 인스턴스 생성
  → RecoveryRepository Provider 등록
  → SyncManagerImpl 생성 시 recovery DataSource 주입
  → OperationRouter 생성 시 recovery DAO 주입
```

#### 수정된 체크리스트

##### Phase 1: 데이터 모델 + DB (v13)
- [ ] `lib/data/models/outbox_operation.dart` - enum 확장 (recoveryEpisode, recoveryEvent)
- [ ] `lib/data/models/meal_diary.dart` - recoveryEpisodeUuid 필드 추가
- [ ] `lib/data/models/recovery_episode.dart` - freezed 모델 생성
- [ ] `lib/data/models/recovery_event.dart` - freezed 모델 생성
- [ ] `lib/data/local/drift/tables/meal_diary_table.dart` - recoveryEpisodeUuid 컬럼 추가
- [ ] `lib/data/local/drift/tables/recovery_episode_table.dart` - Drift 테이블
- [ ] `lib/data/local/drift/tables/recovery_event_table.dart` - Drift 테이블
- [ ] `lib/data/local/database/migrations/migration_v13.dart` - 마이그레이션
- [ ] `lib/data/local/database_provider.dart` - migrateToV13 import + onUpgrade
- [ ] `lib/data/local/database/schema_definitions.dart` - 신규 설치용 테이블 정의
- [ ] `lib/data/local/drift/app_database.dart` - 테이블 등록 + schemaVersion 13
- [ ] `lib/data/local/database_config.dart` - version = 13

##### Phase 2: DAO
- [ ] `lib/data/local/recovery_episode_dao.dart` - BaseDao 확장
- [ ] `lib/data/local/recovery_event_dao.dart` - BaseDao 확장
- [ ] `lib/data/local/database_helper.dart` - DAO 인스턴스 추가
- [ ] `lib/data/local/meal_diary_dao.dart` - toMap 수정 + linkRecoveryEpisodeIfNeeded()

##### Phase 3: Repository
- [ ] `lib/data/repositories/recovery_repository.dart` - 비즈니스 로직

##### Phase 4: 동기화
- [ ] `lib/data/remote/firestore_recovery_episode_data_source.dart`
- [ ] `lib/data/remote/firestore_recovery_event_data_source.dart`
- [ ] `lib/data/services/operation_router.dart` - 생성자 수정 + 핸들러 추가
- [ ] `lib/data/services/sync_manager_impl.dart` - 생성자 수정 + Pull 타겟 추가 (순차)

##### Phase 5: DI 와이어링
- [ ] `lib/main.dart` - RecoveryRepository + DataSource 주입

##### Phase 6: 검증
- [ ] `flutter pub run build_runner build --delete-conflicting-outputs`
- [ ] `flutter analyze` 통과
- [ ] 마이그레이션 테스트 (v12 → v13)

---

## 작업 로그

### 2026-01-28 17:10 - Codex 리뷰 완료
- 구현 계획 v1 작성 후 Codex 검증 실행
- 6개 이슈 발견 (DB 버전 충돌, 스키마 누락, DI 와이어링 등)
- 계획 v2로 수정 완료
- 다음 단계: `/do-dev-task` 재실행하여 구현 진행

---

## 짧은 회고
