...
export const CalendarSidebar: React.FC = () => {
    ...
    return (
        <div
            className={`
                flex-shrink-0 bg-white border-r border-gray-200 h-full transition-all duration-300 ease-in-out relative
                ${isSidebarCollapsed ? 'w-12' : 'w-[260px]'}
            `}
        >
            {/* Collapse Toggle Button */}
            <button
                onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
                className="absolute -right-3 top-20 z-10 w-6 h-6 bg-white border border-gray-200 rounded-full flex items-center justify-center text-gray-500 hover:text-gray-800 shadow-sm transition-transform hover:scale-110"
                title={isSidebarCollapsed ? "Expand Sidebar" : "Collapse Sidebar"}
            >
                <span className={`text-[10px] transition-transform duration-300 ${isSidebarCollapsed ? 'rotate-180' : ''}`}>
                    ◀
                </span>
            </button>

            <div className={`h-full overflow-hidden ${isSidebarCollapsed ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
                <div className="h-full overflow-y-auto w-[260px]">
            ...
            {/* Icon only bar when collapsed */}
            {isSidebarCollapsed && (
                <div className="absolute inset-0 flex flex-col items-center pt-20 gap-4 pointer-events-none">
                    <span className="text-lg opacity-50 grayscale">📅</span>
                    <div className="flex flex-col gap-2">
                        {accounts?.map(account => (
                            <div key={account.accountId} className="w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-[10px] font-bold text-gray-500">
                                {account.email.charAt(0).toUpperCase()}
                            </div>
                        ))}
                    </div>
