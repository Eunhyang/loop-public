...
### 잘못된 가설 예시 (REJECT)
- "API 응답 속도를 개선하면 성능이 좋아지는가?" → 기술 가설, 고객 부재
- "알고리즘이 정확도를 높일 수 있는가?" → 기술 주어, 고객 문제 없음

### 올바른 가설 예시 (ACCEPT)
- "12주 과정 참여자가 첫 주에 습관 설정 시 코칭 AI의 리마인더를 받으면 2주차 지속률이 30% 이상 향상되는가?"
- "습관 앱 사용자가 3일 연속 실패 후 개인화된 격려 메시지를 받으면 재시작률이 증가하는가?"
...
2. **hypothesis_question**: 질문 형태 (**반드시 ?로 끝나야 하고, 위 4요소 포함**)
   - 예: "12주 과정 참여자가 첫 주에 습관 설정 시 코칭 AI의 리마인더를 받으면 2주차 지속률이 30% 이상 향상되는가?"
