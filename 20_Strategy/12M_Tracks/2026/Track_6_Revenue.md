...
### Focus 1: 유료 구독 (보험형 가치)
**현재**: 부분 유료
**목표**: 전면 유료화
**진행률**: 30%

**전면 유료화 정의**:
> **전면 유료화 = 프리미엄 레이어(대시보드/인사이트/개인화/리플레이) 전면 유료화이며, Emergency Brake는 항상 접근 가능**

**유료 구독 구조**:
- **Episode Brake(개입) + Recovery(리바운드 차단) + Insight(패턴/예측)** 패키지
- 월 9,900원 ~ 19,900원
- 연간 구독 할인
...
> ⚠️ **Emergency Brake Free 범위**: Quick Brake(15~30초) + 회복 시작(첫 1분)까지만 Free.
> - 이 범위를 넓히면 → 수익 불가
> - 이 범위를 좁히면 → PMF 붕괴

**왜 이 구조인가?**
- ❌ "매일 써야 값어치" (습관앱 구독)
- ✅ "필요할 때 확실히 작동" (안전장치/보험 가치)
- 결제 이유: 없으면 불안, 대체재 없음, 사건 때 돌아옴

**유료 가치 근거**:
- TTR (Time-to-Relief) 단축
- Relapse Interval 증가
- PUPI (없으면 불안) 언어 수렴
...
> - 행동 시그널: (1) 트리거 시 1순위 오픈 OR (2) 90일 내 재방문 / 30일 내 2회+ 복귀

**목표**:
- 전체 전환율 5%
- **PUPI 높은 코호트 전환율**: 15%+ (별도 추적)
...
| Track | 기여 내용 |
|-------|----------|
| Track 1 (Product) | Episode Brake = 유료화 핵심 (보험형 가치, Quick Brake + 회복 시작 Free) |
...
- id: prj-app-kkokkkok
  type: Project
  title: 꼭꼭앱 Release Train
  attrs:
    gtm_role: activation_product
    owner: Product Team
...
- id: trk-6
  ...
  attrs:
    gtm_role: revenue_track


### Edges
- type: gtm_activates_into
  from_id: prj-content-youtube
  to_id: prj-app-kkokkkok
  note: 콘텐츠 유입을 앱 설치/활성으로 전환
  evidence: "YouTube CTA/링크/설치 유도 퍼널 (랜딩/스토어/딥링크)"

- type: gtm_upsells_to
  from_id: prj-app-kkokkkok
  to_id: prj-app-subscription
  note: 앱 사용자를 유료 구독으로 업셀
  evidence: "앱 내 paywall/리포트/성과 가시화 기반 전환"

- type: gtm_monetized_by
  from_id: prj-app-subscription
  to_id: trk-6
  note: 구독 매출이 Revenue Track KPI를 구성
  evidence: "RevenueCat/결제 지표가 Track_6_Revenue KPI로 집계"