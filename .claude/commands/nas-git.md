...
git add -A
git diff --cached --quiet || git commit --no-verify -m "nas-auto: $(date '\''+%Y-%m-%d %H:%M'\'')"
git push origin main 2>&1
'

# Step 2: 로컬 pull
echo ""
echo "=== Step 2: 로컬 pull (충돌 시 여기서 해결) ==="
git pull --rebase origin main

# Step 3: 로컬 commit + push
echo ""
echo "=== Step 3: 로컬 commit + push ==="
git add -A
git diff --cached --quiet || git commit --no-verify -m "local-sync: $(date '+%Y-%m-%d %H:%M')"
git push origin main

# Step 4: NAS pull
echo ""
echo "=== Step 4: NAS pull ==="
sshpass -p 'Dkssud272902*' ssh -p 22 -o StrictHostKeyChecking=no Sosilab@100.93.242.60 '
cd /volume1/LOOP_CORE/vault/LOOP
git config --global --add safe.directory /volume1/LOOP_CORE/vault/LOOP 2>/dev/null
...
echo "=== 변경사항 커밋 ==="
git add -A
git diff --cached --quiet || git commit --no-verify -m "manual: $(date '\''+%Y-%m-%d %H:%M'\'')"
...
- 충돌 발생 시 `local-sync`의 Step 2에서 로컬에서 해결 (NAS보다 편함)
